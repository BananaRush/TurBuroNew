...
(function () {
	...
	function listener(event) {
		...
		//alert(result.url);
		window.CKEDITOR.tools.callFunction(result.urlParam, result.url);
		alert("получено: " + event.data);
		//window.CKEDITOR.tools.callFunction(0, event.data);
	}
	...
	window.CKEDITOR && window.CKEDITOR.dom || (window.CKEDITOR || (window.CKEDITOR = function () {
			...
			d = {
				...
				domReady: function () {
					...
					return function (e) {
						...
						if (1 == b.length) if (document.addEventListener) document.addEventListener('DOMContentLoaded', a, !1),
		...
		return {
			...
			on: function (c, b, d, k, e) {
				...
				if (0 > f.getListenerIndex(b)) {
					...
					h.fn = b;
					h.priority = e;
					for (var p = f.length - 1; 0 <= p; p--) if (f[p].priority <= e) return f.splice(p + 1, 0, h),
						{
							removeListener: m
						};
					f.unshift(h)
		...
		b.gecko && (d = a.match(/rv:([\d\.]+)/)) && (d = d[1].split('.'), c = 10000 * d[0] + 100 * (d[1] || 0) + 1 * (d[2] || 0));
		b.air && (c = parseFloat(a.match(/ adobeair\/(\d+)/)[1]));
		b.webkit && (c = parseFloat(a.match(/ applewebkit\/(\d+)/)[1]));
		b.version = c;
		b.isCompatible = !(b.ie && 7 > c) && !(b.gecko && 40000 > c) && !(b.webkit && 534 > c);
		...
		CKEDITOR.tools = {
			...
			buildStyleHtml: function (a) {
				...
				for (var f, e = [
				], b = 0; b < a.length; b++) if (f = a[b]) /@import|[{}]/.test(f) ? e.push('<style>' +
					f + '</style>') : e.push('<link type="text/css" rel=stylesheet href="' + f + '">');
				return e.join('')
			},
			htmlEncode: function (a) {
				return void 0 === a || null === a ? '' : String(a).replace(b, '&amp;').replace(c, '&gt;').replace(g, '&lt;')
			},
			...
			transformPlainTextToHtml: function (a, f) {
				...
				if (!e) {
					...
					if (c.test(b)) var g = '<' + h + '>',
						d = '</' + h + '>',
						b = g + b.replace(c, function () {
							return d + g
						}) + d
				}
				b = b.replace(/\n/g, '<br>');
				e || (b = b.replace(new RegExp('<br>(?=</' + h + '>)'), function (f) {
			...
			convertRgbToHex: function (a) {
				return a.replace(/(?:rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\))/gi, function (f, a, e, m) {
					f = [
						a,
						e,
						m
			...
			normalizeHex: function (a) {
				return a.replace(/#(([0-9a-f]{3}){1,2})($|;|\s+)/gi, function (f, a, e, m) {
					f = a.toLowerCase();
					3 == f.length && (f = f.split(''), f = [
						f[0],
						f[0],
						f[1],
						f[1],
			...
			parseCssText: function (a, f, e) {
				...
				a.replace(/&quot;/g, '"').replace(/\s*([^:;\s]+)\s*:\s*([^;]+)\s*(?=;|$)/g, function (a, e, m) {
					f && (e = e.toLowerCase(), 'font-family' == e && (m = m.replace(/\s*,\s*/g, ',')), m = CKEDITOR.tools.trim(m));
					b[e] = m
				});
				return b
			},
			...
			enableHtml5Elements: function (a, f) {
				for (var e = 'abbr article aside audio bdi canvas data datalist details figcaption figure footer header hgroup main mark meter nav output progress section summary time video'.split(' '), b = e.length, h; b--;) h = a.createElement(e[b]),
					f && a.appendChild(h)
			},
			...
			getCookie: function (a) {
				a = a.toLowerCase();
				for (var f = document.cookie.split(';'), e, b, h = 0; h < f.length; h++) if (e = f[h].split('='), b = decodeURIComponent(CKEDITOR.tools.trim(e[0]).toLowerCase()), b === a) return decodeURIComponent(1 < e.length ? e[1] : '');
				return null
			},
			setCookie: function (a, f) {
				document.cookie = encodeURIComponent(a) + '=' + encodeURIComponent(f) + ';path=/'
			},
				...
				parse: {
					...
					margin: function (a) {
						...
						var e = {
							...
							b = a.match(/(?:\-?[\.\d]+(?:%|\w*)|auto|inherit|initial|unset)/g) || [
								'0px'
							];
						switch (b.length) {
							case 1:
			...
			array: {
				...
				map: function (a, f, e) {
					for (var b = [
					], h = 0; h < a.length; h++) b.push(f.call(e, a[h], h, a));
					return b
				},
		...
		CKEDITOR.tools.extend(CKEDITOR.dom.node.prototype, {
			...
			getAddress: function (a) {
				for (var d = [
				], b = this.getDocument().$.documentElement, c = this.$; c && c != b;) {
					var g = c.parentNode;
					g && d.unshift(this.getIndex.call({
						$: c
					}, a));
					c = g
				}
			...
			getIndex: function (a) {
				...
				do a && c != this.$ && c.nodeType == CKEDITOR.NODE_TEXT && (l || b(c)) || (g++ , l = c.nodeType == CKEDITOR.NODE_TEXT);
		...
		CKEDITOR.tools.extend(CKEDITOR.dom.document.prototype, {
			...
			getByAddress: function (a, d) {
				for (var b = this.$.documentElement, c = 0; b && c < a.length; c++) {
					var g = a[c];
					if (d) for (var l = - 1, k = 0; k < b.childNodes.length; k++) {
		...
		CKEDITOR.dom.element.setMarker = function (a, d, b, c) {
			var g = d.getCustomData('list_marker_id') || d.setCustomData('list_marker_id', CKEDITOR.tools.getNextNumber()).getCustomData('list_marker_id'),
				l = d.getCustomData('list_marker_names') || d.setCustomData('list_marker_names', {
			...
			CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
				...
				removeClass: g ? function (a) {
					...
					: function (e) {
						...
						b && a(b, e) && ((b = b.replace(new RegExp('(?:^|\\s+)' +
							e + '(?=\\s|$)'), '').replace(/^\s+/, '')) ? this.setAttribute('class', b) : this.removeAttribute('class'));
						return this
					},
				hasClass: function (e) {
				...
				breakParent: function (a, b) {
					...
					if (CKEDITOR.env.ie && !CKEDITOR.env.edge) {
						for (c = new CKEDITOR.dom.element('div'); g = f.getFirst();) g.$.style.backgroundColor && (g.$.style.backgroundColor = g.$.style.backgroundColor),
							c.append(g);
						c.insertAfter(this);
						c.remove(!0)
				...
				getClientRect: function (a) {
					function b(a) {
						if (a) {
							var e = a.getClientRect();
							c.top += e.top;
							c.left += e.left;
							c.x += e.x;
							c.y += e.y;
							b(a.getWindow().getFrame())
						}
					}
					...
					if (!a) return c;
					b(this.getWindow().getFrame());
					a = CKEDITOR.document.getWindow().getScrollPosition();
					c.top += a.y;
					c.left += a.x;
					c.y += a.y;
					c.x += a.x;
				...
				mergeSiblings: function () {
					function a(b, e, f) {
						if (e && e.type == CKEDITOR.NODE_ELEMENT) {
							for (var c = [
							]; e.data('cke-bookmark') || e.isEmptyInlineRemoveable();) if (c.push(e), e = f ? e.getNext() : e.getPrevious(), !e || e.type != CKEDITOR.NODE_ELEMENT) return;
							if (b.isIdentical(e)) {
								for (var g = f ? b.getLast() : b.getFirst(); c.length;) c.shift().move(b, !f);
								e.moveChildren(b, !f);
								e.remove();
				...
				removeStyle: function (a) {
					...
					if (b.removeProperty || 'border' != a && 'margin' != a && 'padding' != a) b.removeProperty ? b.removeProperty(a) : b.removeAttribute(CKEDITOR.tools.cssStyleToDomStyle(a)),
						...
						for (var b = [
						], g = 0; g < c.length; g++) if (f) for (var d = 0; d < f.length; d++) b.push([a,
							c[g],
							f[d]].join('-'));
							else b.push([a,
								c[g]].join('-'));
				...
				getDocumentPosition: function (a) {
					...
					if (document.documentElement.getBoundingClientRect && (CKEDITOR.env.ie ? 8 !== CKEDITOR.env.version : 1)) {
						var k = this.$.getBoundingClientRect(),
							l = f.$.documentElement,
							w = l.clientTop || g.$.clientTop || 0,
							t = l.clientLeft || g.$.clientLeft || 0,
							u = !0;
						CKEDITOR.env.ie && (u = f.getDocumentElement().contains(this), f = f.getBody().contains(this), u = d && f || !d && u);
						u && (CKEDITOR.env.webkit || CKEDITOR.env.ie && 12 <= CKEDITOR.env.version ? (b = g.$.scrollLeft || l.scrollLeft, c = g.$.scrollTop || l.scrollTop) : (c = d ? g.$ : l, b = c.scrollLeft, c = c.scrollTop), b = k.left + b - t, c = k.top + c - w)
					} else for (w = this, t = null; w && 'body' != w.getName() && 'html' != w.getName();) {
						b += w.$.offsetLeft - w.$.scrollLeft;
						c += w.$.offsetTop -
							w.$.scrollTop;
						w.equals(this) || (b += w.$.clientLeft || 0, c += w.$.clientTop || 0);
						for (; t && !t.equals(w);) b -= t.$.scrollLeft,
							c -= t.$.scrollTop,
							t = t.getParent();
						t = w;
				...
				copyAttributes: function (a, b) {
					...
					for (var f = 0; f < c.length; f++) {
						var g = c[f],
							...
							k;
						if (!(d in b)) if ('checked' == d && (k = this.getAttribute(d))) a.setAttribute(d, k);
						else if (!CKEDITOR.env.ie || this.hasAttribute(d)) k = this.getAttribute(d),
							null === k && (k = g.nodeValue),
							a.setAttribute(d, k)
					}
				...
				forEach: function (a, b, c) {
					...
					if (!1 !== f) {
						c = this.getChildren();
						for (var g = 0; g < c.count(); g++) f = c.getItem(g),
		...
		function () {
			function a(a, f) {
				...
				if (!a && !this._.guardLTR) {
					...
					this._.guardLTR = function (a, f) {
						return (!f || !l.equals(a)) && (!A || !a.equals(A)) && (a.type !=
							CKEDITOR.NODE_ELEMENT || !f || !a.equals(b.root))
					}
				}
				if (a && !this._.guardRTL) {
					var G = e.type == CKEDITOR.NODE_ELEMENT ? e : e.getParent(),
						D = e.type == CKEDITOR.NODE_ELEMENT ? g ? e.getChild(g - 1) : null : e.getPrevious();
					this._.guardRTL = function (a, f) {
						return (!f || !G.equals(a)) && (!D || !a.equals(D)) && (a.type != CKEDITOR.NODE_ELEMENT || !f || !a.equals(b.root))
					}
				}
				var F = a ? this._.guardRTL : this._.guardLTR;
				h = m ? function (a, f) {
					return !1 === F(a, f) ? !1 : m(a, f)
				}
					: F;
				this.current ? c = this.current[k](!1, n, h) : (a ? c.type == CKEDITOR.NODE_ELEMENT && (c = 0 < d ? c.getChild(d -
					1) : !1 === h(c, !0) ? null : c.getPreviousSourceNode(!0, n, h)) : (c = e, c.type == CKEDITOR.NODE_ELEMENT && ((c = c.getChild(g)) || (c = !1 === h(e, !0) ? null : e.getNextSourceNode(!0, n, h)))), c && !1 === h(c) && (c = null));
				for (; c && !this._.end;) {
					this.current = c;
					if (!this.evaluator || !1 !== this.evaluator(c)) {
						if (!f) return c
					} else if (f && this.evaluator) return !1;
					c = c[k](!1, n, h)
				}
				this.end();
			...
			function d(f) {
				for (var b, c = null; b = a.call(this, f);) c = b;
				return c
			}
			...
			CKEDITOR.dom.walker.blockBoundary = function (a) {
				return function (f) {
					return !(f.type == CKEDITOR.NODE_ELEMENT && f.isBlockBoundary(a))
				}
			};
			...
			CKEDITOR.dom.walker.invisible = function (a) {
				...
				return function (c) {
					f(c) ? c = 1 : (c.type == CKEDITOR.NODE_TEXT && (c = c.getParent()), c = c.$.offsetWidth <= b);
					return !!(a ^ c)
				}
			};
			CKEDITOR.dom.walker.nodeType = function (a, f) {
				return function (b) {
					return !!(f ^ b.type == a)
				}
			};
			CKEDITOR.dom.walker.bogus = function (a) {
				...
				return function (b) {
					...
					c && (c = b.getParent(), b = b.getNext(f), c = c.isBlockBoundary() && (!b || b.type == CKEDITOR.NODE_ELEMENT && b.isBlockBoundary()));
					return !!(a ^ c)
				}
			};
		...
		function () {
			...
			function d(a, b, c, e, g) {
				function d(a, f, b, c) {
					var e = b ? a.getPrevious() : a.getNext();
					if (c && k) return e;
					x || c ? f.append(a.clone(!0, g), b) : (a.remove(), l && f.append(a, b));
					return e
				}
				function h() {
					var a,
						f,
						b,
						c = Math.min(J.length, E.length);
					for (a = 0; a < c; a++) if (f = J[a], b = E[a], !f.equals(b)) return a;
					return a - 1
				}
				function m() {
					var b = R - 1,
				...
				if (x && C.type == CKEDITOR.NODE_TEXT && (y.equals(C) || y.type === CKEDITOR.NODE_ELEMENT && y.getFirst().equals(C))) c.append(a.document.createText(C.substring(z, A)));
					...
					y.type == CKEDITOR.NODE_TEXT ? x ? H = !0 : y.split(z) : 0 < y.getChildCount() ?
						0 === z ? (y = y.getChild(z), G = !0) : y = y.getChild(z - 1) : F = G = !0;
					for (var J = y.getParents(), E = C.getParents(), R = h(), O = J.length - 1, S = E.length - 1, L = c, V, Z, X, da = - 1, P = R; P <= O; P++) {
						Z = J[P];
						X = Z.getNext();
						for (P != O || Z.equals(E[P]) && O < S ? b && (V = L.append(Z.clone(0, g))) : G ? d(Z, L, !1, F) : H && L.append(a.document.createText(Z.substring(z))); X;) {
							if (X.equals(E[P])) {
								da = P;
								break
							}
					...
					L = c;
					for (P = R; P <= S; P++) if (c = E[P], X = c.getPrevious(), c.equals(J[P])) b && (L = L.getChild(0));
					else {
						P != S || c.equals(J[P]) && S < O ? b && (V = L.append(c.clone(0, g))) : D ? d(c, L, !1, I) : K && L.append(a.document.createText(c.substring(0, A)));
			...
			CKEDITOR.dom.range.prototype = {
				...
				createBookmark: function (a) {
					...
					d || (c = b.clone(), c.setHtml('&nbsp;'), a && c.setAttribute('id', e + 'E'), g = this.clone(), g.collapse(), g.insertNode(c));
					g = this.clone();
					g.collapse(!0);
					g.insertNode(b);
				...
				getBoundaryNodes: function () {
					var a = this.startContainer,
						...
						e = this.endOffset,
						g;
					if (a.type == CKEDITOR.NODE_ELEMENT) if (g = a.getChildCount(), g > c) a = a.getChild(c);
					...
					if (b.type == CKEDITOR.NODE_ELEMENT) if (g = b.getChildCount(), g > e) b = b.getChild(e).getPreviousSourceNode(!0);
				...
				trim: function (a, b) {
					...
					if ((!a || g) && c && c.type == CKEDITOR.NODE_TEXT) {
						if (e) if (e >= c.getLength()) e = c.getIndex() + 1,
							c = c.getParent();
						else {
							var d = c.split(e),
								e = c.getIndex() + 1,
				...
				enlarge: function (a, b) {
					...
					switch (a) {
						...
						case CKEDITOR.ENLARGE_ELEMENT:
							...
							A.type == CKEDITOR.NODE_TEXT ? (D && (A = !CKEDITOR.tools.trim(A.substring(0, D)).length && A, z = !!A), A && ((y = A.getPrevious()) || (x = A.getParent()))) : (D && (y = A.getChild(D - 1) || A.getLast()), y || (x = A));
							for (x = c(x); x || y;) {
								if (x && !y) {
									!C && x.equals(h) && (C = !0);
								...
								for (; y;) if (A = !1, y.type == CKEDITOR.NODE_COMMENT) y = y.getPrevious();
								else {
									if (y.type == CKEDITOR.NODE_TEXT) G = y.getText(),
										e.test(G) && (y = null),
										A = /[\s\ufeff]$/.test(G);
									else if ((y.$.offsetWidth > (CKEDITOR.env.webkit ? 1 :
										0) || b && y.is('br')) && !y.data('cke-bookmark')) if (z && CKEDITOR.dtd.$removeEmpty[y.getName()]) {
											G = y.getText();
											if (e.test(G)) y = null;
											else for (var D = y.$.getElementsByTagName('*'), F = 0, I; I = D[F++];) if (!CKEDITOR.dtd.$removeEmpty[I.nodeName.toLowerCase()]) {
												y = null;
												break
											}
											y && (A = !!G.length)
								...
								for (; y;) {
									...
									if (y.type == CKEDITOR.NODE_TEXT) G = y.getText(),
										...
										if ((0 < y.$.offsetWidth || b && y.is('br')) && !y.data('cke-bookmark')) if (z && CKEDITOR.dtd.$removeEmpty[y.getName()]) {
											G = y.getText();
											if (e.test(G)) y = null;
											else for (D = y.$.getElementsByTagName('*'), F = 0; I = D[F++];) if (!CKEDITOR.dtd.$removeEmpty[I.nodeName.toLowerCase()]) {
												y = null;
												break
											}
											y && (A = !!G.length)
						...
						case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:
							x = new CKEDITOR.dom.range(this.root);
							m = this.root;
							...
							x = this.clone();
							x.collapse();
							x.setEndAt(m, CKEDITOR.POSITION_BEFORE_END);
							x = new CKEDITOR.dom.walker(x);
				...
				shrink: function (a, b, c) {
					...
					if (!this.collapsed) {
						...
						d.guard = function (b, c) {
							...
							if (a == CKEDITOR.SHRINK_ELEMENT && b.type == CKEDITOR.NODE_TEXT || c && b.equals(z) || !1 === e && b.type == CKEDITOR.NODE_ELEMENT && b.isBlockBoundary() || b.type == CKEDITOR.NODE_ELEMENT && b.hasAttribute('contenteditable')) return !1;
				...
				splitBlock: function (a, b) {
					...
					this.deleteContents();
					g && g.equals(d) && (e ? (h = new CKEDITOR.dom.elementPath(this.startContainer, this.root), this.moveToPosition(d, CKEDITOR.POSITION_AFTER_END), d = null) : c ? (h = new CKEDITOR.dom.elementPath(this.startContainer, this.root), this.moveToPosition(g, CKEDITOR.POSITION_BEFORE_START), g = null) : (d = this.splitElement(g, b || !1), g.is('ul', 'ol') || g.appendBogus()));
					return {
						previousBlock: g,
						nextBlock: d,
						wasStartOfBlock: c,
				...
				removeEmptyBlocksAtEnd: function () {
					function a(f) {
						return function (a) {
							return b(a) || c(a) || a.type == CKEDITOR.NODE_ELEMENT && a.isEmptyInlineRemoveable() || f.is('table') && a.is('caption') ? !1 : !0
						}
					}
					var b = CKEDITOR.dom.walker.whitespaces(),
						c = CKEDITOR.dom.walker.bookmark(!1);
					return function (b) {
						for (var c = this.createBookmark(), e = this[b ? 'endPath' : 'startPath'](), g = e.block || e.blockLimit, d; g && !g.equals(e.root) && !g.getFirst(a(g));) d = g.getParent(),
				...
				checkStartOfBlock: function () {
					var a = this.startContainer,
						c = this.startOffset;
					CKEDITOR.env.ie && c && a.type == CKEDITOR.NODE_TEXT && (a = CKEDITOR.tools.ltrim(a.substring(0, c)), e.test(a) && this.trim(0, 1));
					this.trim();
				...
				moveToClosestEditablePosition: function (a, b) {
					...
					else if (g = c[b ? 'getNextEditableNode' : 'getPreviousEditableNode']()) e = 1,
				...
				scrollIntoView: function () {
					var a = new CKEDITOR.dom.element.createFromHtml('<span>&nbsp;</span>', this.document),
						...
						g = this.clone();
					g.optimize();
					(e = g.startContainer.type == CKEDITOR.NODE_TEXT) ? (c = g.startContainer.getText(), b = g.startContainer.split(g.startOffset), a.insertAfter(g.startContainer)) : g.insertNode(a);
					a.scrollIntoView();
				...
				_find: function (a, b) {
					var c = this.getCommonAncestor(),
						...
						m,
						k;
					if (c && c.find) for (h = c.find(a), d = 0; d < h.count(); d++) if (c = h.getItem(d), b || !c.isReadOnly()) m = c.getPosition(e.startNode) & CKEDITOR.POSITION_FOLLOWING || e.startNode.equals(c),
						k = c.getPosition(e.endNode) & CKEDITOR.POSITION_PRECEDING + CKEDITOR.POSITION_IS_CONTAINED || e.endNode.equals(c),
						m && k && g.push(c);
					return g
				}
			};
			...
			a.prototype = {
				getNextParagraph: function (a) {
					...
					if (this._.nestedEditable) {
						if (f = this._.nestedEditable.iterator.getNextParagraph(a)) return this.activeFilter = this._.nestedEditable.iterator.activeFilter,
							f;
					...
					if (!this._.started) {
						...
						d = w.startPath();
						var t = w.endPath(),
							u = !w.collapsed && c(w, d.block),
							r = !w.collapsed && c(w, t.block, 1);
						w.shrink(CKEDITOR.SHRINK_ELEMENT, !0);
						u && w.setStartAt(d.block, CKEDITOR.POSITION_BEFORE_END);
						r && w.setEndAt(t.block, CKEDITOR.POSITION_AFTER_START);
					...
					for (this._.nextNode = null; t;) {
						...
						if (B) t.type == CKEDITOR.NODE_TEXT && g.test(t.getText()) && (B = 0);
							...
							if (CKEDITOR.dtd.$block[y] && 'false' == t.getAttribute('contenteditable')) {
								...
								if ('br' == y) B = 1;
								else if (!d && !t.getChildCount() && 'hr' != y) {
									f = t;
									k = t.equals(w);
									break
								}
								d && (d.setEndAt(t, CKEDITOR.POSITION_BEFORE_START), 'br' != y && (this._.nextNode = t));
								u = 1
							} else {
								if (t.getFirst()) {
									d || (d = this.range.clone(), d.setStartAt(t, CKEDITOR.POSITION_BEFORE_START));
									t = t.getFirst();
									continue
								}
								B = 1
						...
						t = this._getNextSourceNode(t, x, w);
						if ((k = !t) || u && d) break
					}
					if (!f) {
						if (!d) return this._.docEndMarker && this._.docEndMarker.remove(),
		...
		function () {
			function a(a, b, c, f, e) {
				...
				for (f = a.length; c < f; ++c) {
					...
					else for (z in h = g.elements, delete g.elements, h) if (b[z]) b[z][d ? 'unshift' : 'push'](g);
					else b[z] = [
						g
					]
				}
			...
			function d(a, c, f, g) {
				if (!a.match || a.match(c)) if (g || e(a, c)) if (a.propertiesOnly || (f.valid = !0), f.allAttributes || (f.allAttributes = b(a.attributes, c.attributes, f.validAttributes)), f.allStyles || (f.allStyles = b(a.styles, c.styles, f.validStyles)), !f.allClasses) {
					a = a.classes;
					c = c.classes;
					g = f.validClasses;
					if (a) if (!0 === a) a = !0;
					else {
						for (var d = 0, h = c.length, m; d < h; ++d) m = c[d],
							g[m] || (g[m] = a(m));
						a = !1
					} else a = !1;
					f.allClasses = a
			...
			function c(a, b, c) {
				if (!a.match || a.match(b)) {
					...
					b = b.classes;
					if (a) {
						for (var f = !1, e = !0 === a, d = b.length; d--;) if (e || a(b[d])) b.splice(d, 1),
							f = !0;
						a = f
					} else a = !1;
			...
			function g(a, b) {
				...
				var c = !1,
					f = !0 === a,
					e;
				for (e in b) if (f || a(e)) delete b[e],
					c = !0;
				return c
			}
			function l(a, b, c) {
			...
			function f(a) {
				var b,
					...
					d = 1;
				for (a = I(a); b = a.match(E);) (c = b[2]) ? (f = n(c, 'styles'), e = n(c, 'attrs'), c = n(c, 'classes')) : f = e = c = null,
					g['$' + d++] = {
						elements: b[1],
						classes: c,
						styles: f,
				...
				if (e.doFilter) {
					a: {
						...
						if (a || h) {
							...
							if (h) for (x = 0, E = h.length; x < E; ++x) d(h[x], b, n, l);
							a = n
						} else a = null
					}
					...
					if (!a.allAttributes) for (J in m) E[J] || (D.test(J) ? J == (v = J.replace(/^data-cke-saved-/, '')) || E[v] || (delete m[J], x = !0) : (delete m[J], x = !0));
					if (!a.allStyles || a.hadInvalidStyle) {
						for (J in u) a.allStyles || r[J] ? C.push(J + ':' + u[J]) : x = !0;
						C.length && (m.style = C.sort().join('; '))
					} else z && (m.style = z);
					if (!a.allClasses || a.hadInvalidClass) {
						for (J = 0; J < k.length; ++J) (a.allClasses || h[k[J]]) && n.push(k[J]);
						n.length && (m['class'] = n.sort().join(' '));
						l && n.length < l.split(/\s+/).length && (x = !0)
					} else l && (m['class'] = l);
					x && (g = 1);
					if (!e.skipFinalValidation && !t(b)) return f.push(b),
						1
				}
			...
			function x(a, b, c) {
				var f = a.name;
				if (D.$empty[f] || !a.children.length) 'hr' == f && 'br' == b ? a.replaceWith(r()) : (a.parent && c.push({
					check: 'it',
					el: a.parent
				}), a.remove());
				else if (D.$block[f] || 'tr' == f) if ('br' == b) a.previous && !B(a.previous) && (b = r(), b.insertBefore(a)),
					a.next && !B(a.next) && (b = r(), b.insertAfter(a)),
					...
					b: {
						e = D[b];
						for (var g = 0, d = f.length, h; g < d; ++g) if (h = f[g], h.type == CKEDITOR.NODE_ELEMENT && !e[h.name]) {
							e = !1;
							break b
						}
						e = !0
					...
					if (e) a.name = b,
						...
						e = a.parent;
						for (var g = e.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || 'body' == e.name, m, k, d = f.length; 0 < d;) h = f[--d],
							g && (h.type == CKEDITOR.NODE_TEXT || h.type == CKEDITOR.NODE_ELEMENT && D.$inline[h.name]) ? (m || (m = new CKEDITOR.htmlParser.element(b), m.insertAfter(a), c.push({
								check: 'parent-down',
								el: m
							})), m.add(h, 0)) : (m = null, k = D[e.name] || D.span, h.insertAfter(a), e.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || h.type != CKEDITOR.NODE_ELEMENT || k[h.name] || c.push({
								check: 'el-up',
								el: h
							}));
						a.remove()
			...
			CKEDITOR.filter.prototype = {
				...
				applyTo: function (a, b, c, f) {
					...
					a.forEach(function (a) {
						if (a.type == CKEDITOR.NODE_ELEMENT) {
							...
							if (!b || 'span' != a.name || !~CKEDITOR.tools.objectKeys(a.attributes).join('|').indexOf('data-cke-')) if (h = q(e, a, g, k), h & 1) m = !0;
							...
							a: {
								...
								var l,
									n,
									t;
								if (d) for (n = 0; n < d.length; ++n) if ((t = f.match(d[n])) && t[0].length == f.length) {
									c = !0;
									break a
								}
					...
					for (; l = a.pop();) if (c = l.el, c.parent) switch (n = D[c.parent.name] || D.span, l.check) {
						case 'it':
							D.$removeEmpty[c.name] && !c.children.length ? x(c, f, a) : t(c) || x(c, f, a);
							break;
						case 'el-up':
				...
				addContentForms: function (a) {
					if (!this.disabled && a) {
						...
						if (e) {
							for (b = 0; b < a.length; ++b) f.push(z(a[b], e));
					...
					if (!this.disabled && a) {
						...
						for (e = 0; e < a.length; ++e) {
							...
							for (d = 0; d < b.length; ++d) h = b[d],
								'string' == typeof h ? (h = h.split(/\s*:\s*/), m = h[0], k = null, l = h[1]) : (m = h.check, k = h.left, l = h.right),
								g || (g = h, g = g.element ? g.element : m ? m.match(/^([a-z0-9]+)/i)[0] : g.left.getDefinition().element),
								k instanceof CKEDITOR.style && (k = A(k)),
								c.push({
									check: m == g ? null : m,
									left: k,
				...
				check: function (a, b, c) {
					...
					if ('string' == typeof a) {
						...
						g && !CKEDITOR.tools.isEmpty(g) ? (g = F(g), h.style = CKEDITOR.tools.writeCssText(g, !0)) : g = {
						},
						g = {
							name: e.element,
							attributes: h,
					...
					var h = CKEDITOR.tools.clone(g),
						k = [
						],
						l;
					if (!1 !== b && (l = this._.transformations[g.name])) {
						for (e = 0; e < l.length; ++e) y(this, g, l[e]);
						w(g)
					}
					q(this, h, k, {
						doFilter: !0,
			...
			L = CKEDITOR.filter.transformationsTools = {
				...
				splitMarginShorthand: function (a) {
					...
					if (a.styles.margin) {
						var c = a.styles.margin.match(/(\-?[\.\d]+\w+)/g) || [
							'0px'
						];
						switch (c.length) {
							case 1:
				...
				transform: function (a, b) {
					if ('string' == typeof b) a.name = b;
						...
						var c = b.getDefinition(),
							...
							m;
						a.name = c.element;
						for (g in e) if ('class' == g) for (c = a.classes.join('|'), h = e[g].split(/\s+/); m = h.pop();) - 1 == c.indexOf(m) && a.classes.push(m);
			...
			return {
				load: function (b, c, g, l) {
					...
					if (0 === e) f(!0);
						...
						var n = function (a, b) {
							(b ? h : m).push(a);
							0 >= --e && (l && CKEDITOR.document.getDocumentElement().removeStyle('cursor'), f(b))
						},
							p = function (b, c) {
								a[b] = 1;
								var f = d[b];
		...
		CKEDITOR.resourceManager.prototype = {
			...
			load: function (a, d, b) {
				CKEDITOR.tools.isArray(a) || (a = a ? [
					a
				] : [
					]);
				for (var c = this.loaded, g = this.registered, l = [
				], k = {
				}, e = {
				}, h = 0; h < a.length; h++) {
					var m = a[h];
					if (m) if (c[m] || g[m]) e[m] = this.get(m);
					else {
		...
		function () {
			...
			function l(a, b, c) {
				if (b) {
					var f,
						...
						g = a.commands;
					for (e in g) f = g[e],
						(c || f.contextSensitive) && f.refresh(a, b)
				}
			}
			function k(a) {
			...
			function n(a) {
				var b = a.config,
					c = b.plugins && b.plugins.replace(/\s/g, '') || '',
					f = b.extraPlugins && b.extraPlugins.replace(/\s/g, ''),
					e = b.removePlugins && b.removePlugins.replace(/\s/g, '');
				if (f) var g = new RegExp('(?:^|,)(?:' + f.replace(/,/g, '|') + ')(?=,|$)', 'g'),
					c = c.replace(g, ''),
					c = c + (',' + f);
				if (e) var d = new RegExp('(?:^|,)(?:' + e.replace(/,/g, '|') + ')(?=,|$)', 'g'),
					c = c.replace(d, '');
				...
				CKEDITOR.plugins.load(c.split(','), function (c) {
					...
					for (var h in c) {
						...
						if (n && (x = n.match(d))) for (; n = x.pop();) CKEDITOR.error('editor-plugin-required', {
							plugin: n.replace(',', ''),
							requiredBy: h
						});
						k && !a.lang[h] && (k.split && (k = k.split(',')), 0 <= CKEDITOR.tools.indexOf(k, a.langCode) ? l = a.langCode : (l = a.langCode.replace(/-.*/, ''), l = l != a.langCode && 0 <= CKEDITOR.tools.indexOf(k, l) ? l : 0 <= CKEDITOR.tools.indexOf(k, 'en') ? 'en' : k[0]), m.langEntries && m.langEntries[l] ? (a.lang[h] = m.langEntries[l], l = null) : g.push(CKEDITOR.getUrl(m.path + 'lang/' + l + '.js')));
						e.push(l);
						f.push(m)
					}
					CKEDITOR.scriptLoader.load(g, function () {
						for (var c = [
							...
							'init',
							'afterInit'
						], g = 0; g < c.length; g++) for (var d = 0; d < f.length; d++) {
							var h = f[d];
			...
			CKEDITOR.htmlParser.prototype = {
				...
				parse: function (b) {
					for (var c, g, l = 0, k; c = this._.htmlPartsRegex.exec(b);) {
						g = c.index;
						if (g > l) if (l = b.substring(l, g), k) k.push(l);
						else this.onText(l);
		...
		function () {
			...
			CKEDITOR.htmlParser.fragment.fromHtml = function (l, k, e) {
				...
				function n(b, c, g) {
					c = c || r || w;
					var d = r;
					void 0 === b.previous && (p(c, b) && (r = c, v.onTagOpen(e, {
					}), b.returnPoint = c = r), f(b), a(b) && !b.children.length || c.add(b), 'pre' == b.name && (x = !1), 'textarea' == b.name && (B = !1));
				...
				v.onTagOpen = function (f, e, g, k) {
					...
					if (a(e)) t.push(e);
						...
						if ('br' == f) u.push(e);
							...
							for (; !(k = (g = r.name) ? CKEDITOR.dtd[g] || (r._.isBlockLike ? CKEDITOR.dtd.div : CKEDITOR.dtd.span) : c, e.isUnknown || r.isUnknown || k[f]);) if (r.isOptionalClose) v.onTagClose(g);
							else if (f in b && g in b) g = r.children,
								(g = g[g.length - 1]) && 'li' == g.name || n(g = new CKEDITOR.htmlParser.element('li'), r),
								!e.returnPoint && (e.returnPoint = r),
								r = g;
							else if (f in CKEDITOR.dtd.$listItem && !q(f, g)) v.onTagOpen('li' == f ? 'ul' : 'dl', {
							}, 0, 1);
							else if (g in d && !q(f, g)) !e.returnPoint && (e.returnPoint = r),
								r = r.parent;
				...
				v.onTagClose = function (a) {
					...
					], g = r; g != w && g.name != a;) g._.isBlockLike || f.unshift(g),
						c.push(g),
				...
				v.onText = function (a) {
					if (!(r._.hasInlineStarted && !u.length || x || B) && (a = CKEDITOR.tools.ltrim(a), 0 === a.length)) return;
					var b = r.name,
						f = b ? CKEDITOR.dtd[b] || (r._.isBlockLike ? CKEDITOR.dtd.div : CKEDITOR.dtd.span) : c;
					if (!B && !f['#'] && b in d) v.onTagOpen(g[b] || ''),
						v.onText(a);
					else {
						m();
						h();
		...
		function () {
			...
			a.prototype = {
				...
				execOnName: function (a, c) {
					for (var g = 0, d = this.rules, k = d.length, e; c && g < k; g++) e = d[g],
						a.nonEditable && !e.options.applyToAll || a.nestedEditable && e.options.excludeNestedEditable || (c = c.replace(e.value[0], e.value[1]));
					return c
				}
			}
		}(),
		function () {
					...
					return function (g) {
						if (g.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
							var d = [
								...
								k,
								x;
							if (m) for (h(m, 1) && d.push(m); m;) l(m) && (k = c(m)) && h(k) && ((x = c(k)) && !l(x) ? d.push(k) : (e(n).insertAfter(k), k.remove())),
								m = m.previous;
					...
					return function (b) {
						if (b.parent.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
							...
							if ('data-cke-bogus' in f || 'data-cke-eol' in f) delete f['data-cke-bogus'];
								...
								var d = c(b);
								!f && l(b.parent) ? k(b.parent, e(a)) : l(f) && d && !l(d) && e(a).insertBefore(f)
							}
						}
					}
				}(n);
			...
			function m(a) {
				return a.replace(G, function (a, b, c) {
					return '<' + b + c.replace(D, function (a, b) {
			...
			function f(a, b) {
				return a.replace(b, function (a, b, c) {
					0 === a.indexOf('<textarea') && (a = b + q(c).replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</textarea>');
			...
			CKEDITOR.htmlDataProcessor = function (b) {
				...
				b.on('toHtml', function (a) {
					...
					var c = a.dataValue,
						...
						c = c.replace(R, '<cke:$1$2></cke:$1>'),
						c = c.replace(/(<pre\b[^>]*>)(\r\n|\n)/g, '$1$2$2'),
						c = c.replace(/([^a-z0-9<\-])(on\w{3,})(?!>)/gi, '$1data-cke-' + CKEDITOR.rnd + '-$2');
					e = a.context || b.editable().getName();
					var g;
					CKEDITOR.env.ie && 9 > CKEDITOR.env.version && 'pre' == e && (e = 'div', c = '<pre>' + c + '</pre>', g = 1);
					...
					elements: {
						$: function (a) {
							...
							if (b) {
								...
								for (var c = [
									...
									'href',
									'src'
								], f, e = 0; e < c.length; e++) f = 'data-cke-saved-' + c[e],
									f in b && delete b[c[e]]
							}
							return a
						},
						table: function (a) {
							a.children.slice(0).sort(function (a, b) {
								var c,
									f;
								a.type == CKEDITOR.NODE_ELEMENT && b.type == a.type && (c = CKEDITOR.tools.indexOf(B, a.name), f = CKEDITOR.tools.indexOf(B, b.name));
		...
		function () {
			...
			CKEDITOR.htmlParser.element.prototype = CKEDITOR.tools.extend(new CKEDITOR.htmlParser.node, {
				...
				filter: function (a, b) {
					...
					for (; ;) {
						k = d.name;
						if (!(e = a.onElementName(b, k))) return this.remove(),
							!1;
						d.name = e;
					...
					var h,
						m;
					for (h in k) {
						for (e = k[h]; ;) if (m = a.onAttributeName(b, h)) if (m != h) delete k[h],
							h = m;
						else break;
						else {
							delete k[h];
				...
				find: function (a, b) {
					...
					for (k = 0; k < this.children.length; k++) {
						...
						b && e.find && (d = d.concat(e.find(a, b)))
					}
					return d
				},
				addClass: function (a) {
		...
		function () {
			CKEDITOR.inline = function (a, d) {
				...
				c ? (b.setData(c.getValue(), null, !0), a = CKEDITOR.dom.element.createFromHtml('<div contenteditable="' +
					!!b.readOnly + '" class="cke_textarea_inline">' + c.getValue() + '</div>', CKEDITOR.document), a.insertAfter(c), c.hide(), c.$.form && b._attachToForm()) : b.setData(a.getHtml(), null, !0);
			...
			CKEDITOR.inlineAll = function () {
				var a,
					d,
					b;
				for (b in CKEDITOR.dtd.$editable) for (var c = CKEDITOR.document.getElementsByTag(b), g = 0, l = c.count(); g < l; g++) a = c.getItem(g),
		...
		function () {
			...
			function b(a) {
				var b = a.name,
					...
					m = new CKEDITOR.template('<{outerEl} id="cke_{name}" class="{id} cke cke_reset cke_chrome cke_editor_{name} cke_{langDir} ' + CKEDITOR.env.cssClass + '"  dir="{langDir}" lang="{langCode}" role="application"' + (a.title ? ' aria-labelledby="cke_{name}_arialbl"' : '') + '>' + (a.title ? '<span id="cke_{name}_arialbl" class="cke_voice_label">{voiceLabel}</span>' : '') + '<{outerEl} class="cke_inner cke_reset" role="presentation">{topHtml}<{outerEl} id="{contentId}" class="cke_contents cke_reset" role="presentation"></{outerEl}>{bottomHtml}</{outerEl}></{outerEl}>'),
			...
			CKEDITOR.editor.prototype.setMode = function (a, b) {
				...
				if (a != d.mode && k && k[a]) {
					...
					if (d.mode) {
						...
						'source' == d.mode && k == h && (d.fire('lockSnapshot', {
							forceUpdate: !0
						}), m = 1);
			...
			CKEDITOR.editor.prototype.resize = function (a, b, d, k) {
				...
				k.setSize('width', a, !0);
				m && (m.style.width = '1%');
				var f = (k.$.offsetHeight || 0) - (h.$.clientHeight || 0),
		...
		function () {
			function a(a) {
				var b = a.editor,
					...
					h = g.getRanges()[0],
					m;
				if (CKEDITOR.env.gecko || CKEDITOR.env.ie && CKEDITOR.env.needsBrFiller) if (g = d(g, f)) g.appendBogus(),
					m = CKEDITOR.env.ie;
				...
				}, !e.checkForward() || f.checkStartOfBlock() && f.checkEndOfBlock()) && (b = h.fixBlock(!0, b.activeEnterMode == CKEDITOR.ENTER_DIV ? 'div' : 'p'), CKEDITOR.env.needsBrFiller || (b = b.getFirst(c)) && b.type == CKEDITOR.NODE_TEXT && CKEDITOR.tools.trim(b.getText()).match(/^(?:&nbsp;|\xa0)$/) && b.remove(), m = 1, a.cancel());
				m && h.select()
			}
			function d(a, b) {
			...
			CKEDITOR.editable = CKEDITOR.tools.createClass({
				...
				proto: {
					...
					changeAttr: function (a, b) {
						...
						}), a in this._.attrChanges || (this._.attrChanges[a] = c), this.setAttribute(a, b))
					},
					insertText: function (a) {
						this.editor.focus();
					...
					insertElementIntoRange: function (a, b) {
						...
						b.startContainer.type == CKEDITOR.NODE_ELEMENT && (b.startContainer.is({
							...
							thead: 1,
							tfoot: 1
						}) ? t(b) : b.startContainer.is(CKEDITOR.dtd.$list) && u(b));
						var d,
							h;
					...
					extractHtmlFromRange: function (a, b) {
						...
						if (b) {
							...
							p(a.startContainer) && a.startContainer.appendBogus();
								...
								if (0 !== b.getRanges().length) {
									if (c in n) {
										var g,
											...
											p,
											c = 8 == c;
										CKEDITOR.env.ie && 11 > CKEDITOR.env.version && (g = b.getSelectedElement()) || (g = l(b)) ? (a.fire('saveSnapshot'), d.moveToPosition(g, CKEDITOR.POSITION_BEFORE_START), g.remove(), d.select(), a.fire('saveSnapshot'), e = 1) : d.collapsed && ((m = h.block) && (p = m[c ? 'getPrevious' : 'getNext'](f)) && p.type == CKEDITOR.NODE_ELEMENT && p.is('table') && d[c ? 'checkStartOfBlock' : 'checkEndOfBlock']() ? (a.fire('saveSnapshot'), d[c ? 'checkEndOfBlock' : 'checkStartOfBlock']() && m.remove(), d['moveToElementEdit' + (c ? 'End' : 'Start')](p), d.select(), a.fire('saveSnapshot'), e = 1) : h.blockLimit && h.blockLimit.is('td') && (k = h.blockLimit.getAscendant('table')) && d.checkBoundaryOfElement(k, c ? CKEDITOR.START : CKEDITOR.END) && (p = k[c ? 'getPrevious' : 'getNext'](f)) ? (a.fire('saveSnapshot'), d['moveToElementEdit' + (c ? 'End' : 'Start')](p), d.checkStartOfBlock() && d.checkEndOfBlock() ? p.remove() : d.select(), a.fire('saveSnapshot'), e = 1) : (k = h.contains(['td',
											'th',
											'caption'])) && d.checkBoundaryOfElement(k, c ? CKEDITOR.START : CKEDITOR.END) && (e = 1))
									}
									return !e
								}
							});
							...
							CKEDITOR.env.webkit && this.attachListener(a, 'key', function (b) {
								...
								if (c in n && (b = a.getSelection(), 0 !== b.getRanges().length)) {
									...
									if (f.collapsed) a: {
										...
										d = b.block,
										f = c.endPath().block,
										d && f && !d.equals(f) ? (a.fire('saveSnapshot'), (e = d.getBogus()) && e.remove(), c.enlarge(CKEDITOR.ENLARGE_INLINE), c.deleteContents(), f.getParent() && (f.moveChildren(d, !1), b.lastElement.mergeSiblings(), m(d, f, !0)), c = a.getSelection().getRanges()[0], c.collapse(1), c.optimize(), '' === c.startContainer.getHtml() && c.startContainer.appendBogus(), c.select(), b = !0) : b = !1;
									if (!b) return;
			...
			w = function () {
				...
				function b(c, f) {
					...
					e = f.range.startPath();
					for (var k = n[k.getName()], l = 0, p = c.getChildren(), t = p.count(), r = - 1, u = - 1, v = 0, q = e.contains(n.$list); l < t; ++l) e = p.getItem(l),
						a(e) ? (d = e.getName(), q && d in CKEDITOR.dtd.$list ?
							m = m.concat(b(e, f)) : (h = !!k[d], 'br' != d || !e.data('cke-eol') || l && l != t - 1 || (v = (g = l ? m[l - 1].node : p.getItem(l + 1)) && (!a(g) || !g.is('br')), g = g && a(g) && n.$block[g.getName()]), - 1 != r || h || (r = l), h || (u = l), m.push({
								isElement: 1,
								isLineBreak: v,
				...
				function f(b, c) {
					...
					for (l && e.push(' '); m < d; m++) h = g.getItem(m),
				...
				function h(b, c, f, e) {
					...
					(d = (new CKEDITOR.dom.walker(g)).next()) && a(d) && l[d.getName()] && (m = d.getPrevious()) && a(m) && !m.getParent().equals(b.startContainer) && f.contains(m) && e.contains(d) && d.isIdentical(m) && (d.moveChildren(m), d.remove(), h(b, c, f, e))
				}
				function m(b, c) {
					function f(b, c) {
				...
				return function (l, E, r, u) {
					...
					if (!u.checkReadOnly()) {
						...
						var w,
							I;
						'text' == l.type && E.shrink(CKEDITOR.SHRINK_ELEMENT, !0, !1) && (w = CKEDITOR.dom.element.createFromHtml('<span>&nbsp;</span>', E.document), E.insertNode(w), E.setStartAfter(w));
						...
						w = E.startPath();
						if (v = w.contains(d, !1, 1)) E.splitElement(v),
							l.inlineStylesRoot = v,
						...
						if (w = r) {
							...
							if ('text' == l.type && l.inlineStylesRoot) {
								...
								for (u = l.inlineStylesRoot.getParent(); !I.equals(u);) E = E.appendTo(I.clone()),
									I = I.getParent();
								r = E.getOuterHtml().split('{cke-peak}').join(r)
							}
							I = l.blockLimit.getName();
							if (/^\s+|\s+$/.test(r) && 'span' in CKEDITOR.dtd[I]) {
							...
							if ((P = w.startPath().block) && (1 !=
								P.getChildCount() || !P.getBogus())) a: {
									...
									if (1 == I.getChildCount() && a(Q = I.getFirst()) && Q.is(p) && !Q.hasAttribute('contenteditable')) {
										...
										w = 0;
										for (u = P.count(); w < u; w++) if (E = P.getItem(w), !E.is(t)) break a;
						...
						if (w) {
							...
							for (m(E, Q); u < E.length; u++) {
								...
								if (M) r = 0 < u;
									...
									M = Q.startPath();
									!B.isBlock && k(l.editor, M.block, M.blockLimit) && (N = e(l.editor)) && (N = w.createElement(N), N.appendBogus(), Q.insertNode(N), CKEDITOR.env.needsBrFiller && (U = N.getBogus()) && U.remove(), Q.moveToPosition(N, CKEDITOR.POSITION_BEFORE_END));
									if ((M = Q.startPath().block) && !M.equals(T)) {
										if (U = M.getBogus()) U.remove(),
											P.push(M);
										T = M
									}
									B.firstNotAllowed && (v = 1);
									if (v && B.isElement) {
										M = Q.startContainer;
										...
										if (M) W && (aa = Q.splitElement(W), l.zombies.push(aa), l.zombies.push(W));
											...
											W = f(B.node, W);
											for (var Y = [
											], ca = W.length, ea = 0, ia = void 0, ja = 0, fa = - 1; ea < ca; ea++) ia = W[ea],
												' ' == ia ? (ja || ba && !ea || (Y.push(new CKEDITOR.dom.text(' ')), fa = Y.length), ja = 1) : (Y.push(ia), ja = 0);
											M && fa == Y.length && Y.pop();
											ba = Y
										}
									}
									...
									B.lastNotAllowed && u < E.length - 1 && ((aa = da ? q : aa) && Q.setEndAt(aa, CKEDITOR.POSITION_AFTER_START), v = 0);
									Q.collapse()
								}
							}
							1 != E.length ? U = !1 : (U = E[0], U = U.isElement && 'false' == U.node.getAttribute('contenteditable'));
							U && (r = !0, M = E[0].node, Q.setStartAt(M, CKEDITOR.POSITION_BEFORE_START), Q.setEndAt(M, CKEDITOR.POSITION_AFTER_END));
							l.dontMoveCaret = r;
			...
			t = function () {
				...
				function c(a) {
					var b = a.count(),
						f;
					for (b; 0 < b--;) f = a.getItem(b),
						CKEDITOR.tools.trim(f.getHtml()) || (f.appendBogus(), CKEDITOR.env.ie && 9 > CKEDITOR.env.version && f.getChildCount() && f.getFirst().remove())
				}
				return function (f) {
			...
			r = {
				...
				tree: {
					...
					rebuildFragment: function (a, b, c, f) {
						for (var e; c && !c.equals(b) && f(c);) e = c.clone(0, 1),
			...
			B = function () {
				...
				function b(c, f, e) {
					a(f);
					a(e, 1);
					for (var g; g = e.getNext();) g.insertAfter(f),
						f = g;
					p(c) && c.remove()
				}
				function c(a, b) {
				...
				return {
					list: {
						detectMerge: function (a, b) {
							...
							if (a.mergeList || a.mergeListItems) f = f.clone(),
						...
						merge: function (a, c) {
							if (a.mergeListBookmark) {
								...
								a.mergeListItems && (g = g.contains(CKEDITOR.dtd.$listItem), d = d.contains(CKEDITOR.dtd.$listItem), g.equals(d) || b(d, f, e));
								f.remove();
								e.remove()
							}
						}
					...
					table: function () {
						function a(c) {
							...
							d.guard = function (a, d) {
								...
								if (d && h && a.equals(h)) g = c.clone(),
									g.setEndAt(h, CKEDITOR.POSITION_BEFORE_END),
									e.push(g);
								else if (!d && m && a.equals(m)) g = c.clone(),
						...
						return {
							...
							detectRanges: function (e, g) {
								...
								l = l.contains('table');
								k = k.contains('table');
								if (l || k) l && k && b(l, k) ? (e.tableSurroundingRange = h, h.setStartAt(l, CKEDITOR.POSITION_AFTER_END), h.setEndAt(k, CKEDITOR.POSITION_BEFORE_START), h = d.clone(), h.setEndAt(l, CKEDITOR.POSITION_AFTER_END), m = d.clone(), m.setStartAt(k, CKEDITOR.POSITION_BEFORE_START), m = a(h).concat(a(m))) : l ? k || (e.tableSurroundingRange = h, h.setStartAt(l, CKEDITOR.POSITION_AFTER_END), d.setEndAt(l, CKEDITOR.POSITION_AFTER_END)) : (e.tableSurroundingRange = h, h.setEndAt(k, CKEDITOR.POSITION_BEFORE_START), d.setStartAt(k, CKEDITOR.POSITION_AFTER_START)),
		...
		function () {
			...
			function d(b, c) {
				...
				if ((f = !c && 1 === b.length) && !(f = b[0].collapsed)) {
					...
					var d = g.endContainer.getAscendant({
						td: 1,
						th: 1
					}, !0);
			...
			function b(a) {
				...
				var c = [
					...
					f,
					e;
				for (e = 0; e < a.length; e++) f = a[e]._getTableElement(),
					f.is && f.is({
						td: 1,
						th: 1
				...
				if (b) {
					...
					if (!(f = !c) && (f = !c.isHidden())) {
						...
						if (a(f.getSelectedElement())) f = !1;
							...
							var n = 1 === e.length && e[0].collapsed,
			...
			function h(a) {
				return z(a) || a.type == CKEDITOR.NODE_ELEMENT && !a.is(CKEDITOR.dtd.$empty) ? !0 : !1
			}
			function m(a) {
				function b(c, f) {
			...
			function n(a, b) {
				...
				if (c) {
					...
					d && (c = a.getDocument().$, f = c.getSelection(), c = c.createRange(), c.setStart(d[0].node, d[0].offset), c.collapse(!0), f.removeAllRanges(), f.addRange(c), f.extend(d[1].node, d[1].offset))
				}
			}
			function p(a, b) {
			...
			function q(a, b) {
				var c = CKEDITOR.dom.element.createFromHtml('<div data-cke-hidden-sel="1" data-cke-temp="1" style="' + (CKEDITOR.env.ie && 14 > CKEDITOR.env.version ? 'display:none' : 'position:fixed;top:0;left:-1000px') + '">' + (b || '&nbsp;') + '</div>', a.document);
				a.fire('lockSnapshot');
				a.editable().append(c);
				...
				return function (c) {
					...
					if (b[f]) {
						var e = a.getSelection().getRanges(),
							g = e[0];
						1 == e.length && g.collapsed && (f = g[38 > f ? 'getPreviousEditableNode' :
							'getNextEditableNode']()) && f.type == CKEDITOR.NODE_ELEMENT && 'false' == f.getAttribute('contenteditable') && (a.getSelection().fake(f), c.data.preventDefault(), c.cancel())
					}
				}
			}
			function w(a) {
				for (var b = 0; b < a.length; b++) {
					...
					if (!c.collapsed) {
						if (c.startContainer.isReadOnly()) for (var f = c.startContainer, e; f && !((e = f.type == CKEDITOR.NODE_ELEMENT) && f.is('body') || !f.isReadOnly());) e && 'false' == f.getAttribute('contentEditable') && c.setStartAfter(f),
							f = f.getParent();
						f = c.startContainer;
						e = c.endContainer;
						var g = c.startOffset,
						...
						f.evaluator = function (f) {
							if (f.type == CKEDITOR.NODE_ELEMENT && f.isReadOnly()) {
								...
								f.getPosition(h.endContainer) & CKEDITOR.POSITION_CONTAINS || (e.setStartAfter(f), e.collapsed || a.splice(b +
									1, 0, e));
								return !0
							}
							return !1
			...
			var t = 'function' != typeof window.getSelection,
				...
				y,
				C,
				z = CKEDITOR.dom.walker.invisible(1),
				A = function () {
					function a(b) {
						return function (a) {
					...
					function b(a) {
						return function (b) {
							var c = b.editor,
								f = c.createRange(),
								e;
							if (!c.readOnly) return (e = f.moveToClosestEditablePosition(b.selected, a)) || (e = f.moveToClosestEditablePosition(b.selected, !a)),
								e && c.getSelection().selectRanges([f]),
								c.fire('saveSnapshot'),
								b.selected.remove(),
								e || (f.moveToElementEditablePosition(c.editable()), c.getSelection().selectRanges([f])),
								c.fire('saveSnapshot'),
								!1
						}
					}
			...
			CKEDITOR.on('instanceCreated', function (a) {
				...
				c.on('contentDom', function () {
					...
					CKEDITOR.env.gecko && d.attachListener(d, 'focus', function (a) {
						a.removeListener();
						0 !== r && (a = c.getSelection().getNative()) && a.isCollapsed && a.anchorNode == d.$ && (a = c.createRange(), a.moveToElementEditStart(d), a.select())
					}, null, null, - 2);
					d.attachListener(d, CKEDITOR.env.webkit ? 'DOMFocusIn' : 'focus', function () {
						r && CKEDITOR.env.webkit && (r = c._.previousActive && c._.previousActive.equals(e.getActive())) && null != c._.previousScrollTop && c._.previousScrollTop != d.$.scrollTop && (d.$.scrollTop = c._.previousScrollTop);
						c.unlockSelection(r);
						r = 0
					}, null, null, - 1);
					...
					if (CKEDITOR.env.ie || p) t ? d.attachListener(d, 'beforedeactivate', a, null, null, - 1) : d.attachListener(c, 'selectionCheck', a, null, null, - 1),
								...
								m.on('mousedown', function (a) {
									function b(a) {
										...
										if (y) {
											...
											y.setEndPoint(0 > w.compareEndPoints('StartToStart', c) ? 'EndToEnd' : 'StartToStart', c);
											y.select()
										}
									}
									function c() {
					...
					CKEDITOR.env.webkit && d.attachListener(e, 'keydown', function (a) {
						...
						null,
						null,
						- 1); d.attachListener(d, 'keydown', v(c), null, null, - 1)
				...
				var f,
					e;
				if (a) if (a.getRangeAt) f = (e = a.rangeCount && a.getRangeAt(0)) && new CKEDITOR.dom.node(e.commonAncestorContainer);
				else {
					try {
						e = a.createRange()
					} catch (g) {
				...
				getRanges: function () {
					var a = t ? function () {
						...
						var b = function (b, c) {
							...
							for (var e = f.children, g, d, h = b.duplicate(), m = 0, k = e.length - 1, l = - 1, n, p; m <= k;) if (l = Math.floor((m + k) / 2), g = e[l], h.moveToElementText(g), n = h.compareEndPoints('StartToStart', b), 0 < n) k = l - 1;
							else if (0 > n) m = l + 1;
							else return {
								container: f,
								offset: a(g)
							...
							if (- 1 == l || l == e.length - 1 && 0 > n) {
								...
								for (f = e.length; 0 < h && 0 < f;) d = e[--f],
									d.nodeType == CKEDITOR.NODE_TEXT && (p = d, h -= d.nodeValue.length);
								return {
									container: p,
									offset: - h
								}
							...
							for (; 0 < h;) try {
								d = g[0 < n ? 'previousSibling' : 'nextSibling'],
									d.nodeType == CKEDITOR.NODE_TEXT && (h -= d.nodeValue.length, p = d),
									g = d
							} catch (t) {
								return {
									container: f,
							...
							return {
								container: p,
								offset: 0 < n ? - h : p.nodeValue.length + h
							}
						};
						return function () {
							var a = this.getNative(),
								...
								f = this.getType();
							if (!a) return [];
							if (f == CKEDITOR.SELECTION_TEXT) return a = new CKEDITOR.dom.range(this.root),
								f = b(c, !0),
							...
							if (f == CKEDITOR.SELECTION_ELEMENT) {
								for (var f = [
									...
									for (var g = c.item(e), d = g.parentNode, h = 0, a = new CKEDITOR.dom.range(this.root); h < d.childNodes.length && d.childNodes[h] != g; h++);
				...
				getSelectedText: function () {
					...
					var b = this.getNative(),
						b = t ? 'Control' == b.type ? '' : b.createRange().text :
							b.toString();
				...
				selectRanges: function (a) {
					...
					this.reset();
					if (c) for (var c = this.root, e, h = 0; h < a.length; ++h) e = a[h],
						e.endContainer.equals(c) && (e.endOffset = Math.min(e.endOffset, c.getChildCount()));
					if (a.length) if (this.isLocked) {
						...
						if (l) this.fake(l);
							...
							if (t) {
								...
								if (b.startContainer.type == CKEDITOR.NODE_ELEMENT && b.startContainer.getName() in r || b.endContainer.type == CKEDITOR.NODE_ELEMENT && b.endContainer.getName() in r) b.shrink(CKEDITOR.NODE_ELEMENT, !0),
									a = b.collapsed;
								q = b.createBookmark();
								r = q.startNode;
								a || (k = q.endNode);
								...
								k ? (l = b.document.$.body.createTextRange(), l.moveToElementText(k.$), q.setEndPoint('EndToEnd', l), q.moveEnd('character', - 1)) : (u = r.getNext(p), v = r.hasAscendant('pre'), u = !(u && u.getText && u.getText().match(l)) && (v || !r.hasPrevious() || r.getPrevious().is && r.getPrevious().is('br')), v = b.document.createElement('span'), v.setHtml('&#65279;'), v.insertBefore(r), u && b.document.createText('﻿').insertBefore(r));
								b.setStartBefore(r);
								r.remove();
								a ? (u ? (q.moveStart('character', - 1), q.select(), b.document.$.selection.clear()) : q.select(), b.moveToPosition(v, CKEDITOR.POSITION_BEFORE_START), v.remove()) : (b.setEndBefore(k), k.remove(), q.select())
							} else {
								k = this.getNative();
								if (!k) return;
								...
								for (q = 0; q < a.length; q++) {
									...
									try {
										...
										if (0 <= w.toString().indexOf('NS_ERROR_ILLEGAL_VALUE')) b.collapse(1),
											v.setEnd(b.endContainer.$, b.endOffset);
										else throw w;
									}
									k.addRange(v)
			...
			function a(a, b) {
				for (var c, f; (a = a.getParent()) && !a.equals(b);) if (a.getAttribute('data-nostyle')) c = a;
				else if (!f) {
			...
			function b(c) {
				...
				if (c.collapsed) f = u(this, f),
					...
					for (q.getPosition(t) == CKEDITOR.POSITION_FOLLOWING && (q = 0); q;) {
						k = !1;
						if (q.equals(t)) q = null,
							k = !0;
						else {
							var B = q.type == CKEDITOR.NODE_ELEMENT ? q.getName() : null,
								w = B && 'false' == q.getAttribute('contentEditable'),
								x = B && q.getAttribute('data-nostyle');
							if (B && q.data('cke-bookmark')) {
								q = q.getNextSourceNode(!0);
								continue
							}
							if (w && l && CKEDITOR.dtd.$block[B]) for (var z = q, A = g(z), C = void 0, G = A.length, ea = 0, z = G && new CKEDITOR.dom.range(z.getDocument()); ea <
								G; ++ea) {
								var C = A[ea],
									D = CKEDITOR.filter.instances[C.data('cke-filter')];
								if (D ? D.check(this) : 1) z.selectNodeContents(C),
									b.call(this, z)
							}
							A = B ? !n[B] || x ? 0 : w && !l ? 0 : d(q, t, h, K) : 1;
							if (A) if (C = q.getParent(), A = h, G = e, ea = m, !C || !(C.getDtd() || CKEDITOR.dtd.span)[G] && !ea || A.parentRule && !A.parentRule(C)) k = !0;
							else {
								if (y || B && CKEDITOR.dtd.$removeEmpty[B] && (q.getPosition(t) | K) != K || (y = c.clone(), y.setStartBefore(q)), B = q.type, B == CKEDITOR.NODE_TEXT || w || B == CKEDITOR.NODE_ELEMENT && !q.getChildCount()) {
									for (var B = q, F; (k = !B.getNext(I)) && (F = B.getParent(), n[F.getName()]) && d(F, r, h, J);) B = F;
									y.setEndAfter(B)
								}
							} else k = !0;
						...
						if (k && y && !y.collapsed) {
							for (var k = u(this, f), w = k.hasAttributes(), x = y.getCommonAncestor(), B = {
							}, A = {
							}, C = {
							}, G = {
							}, fa, H, ha; k && x;) {
								if (x.getName() == e) {
									for (fa in h.attributes) !G[fa] && (ha = x.getAttribute(H)) && (k.getAttribute(fa) == ha ? A[fa] = 1 : G[fa] = 1);
									for (H in h.styles) !C[H] && (ha = x.getStyle(H)) && (k.getStyle(H) == ha ? B[H] = 1 : C[H] = 1)
								}
								x = x.getParent()
							}
							for (fa in A) k.removeAttribute(fa);
							for (H in B) k.removeStyle(H);
							w && !k.hasAttributes() && (k = null);
							k ? (y.extractContents().appendTo(k), y.insertNode(k), v.call(this, k), k.mergeSiblings(), CKEDITOR.env.ie || k.$.normalize()) : (k = new CKEDITOR.dom.element('span'), y.extractContents().appendTo(k), y.insertNode(k), v.call(this, k), k.remove(!0));
							y = null
						}
					}
					c.moveToBookmark(p);
			...
			function c(a) {
				...
				if (a.collapsed) {
					for (var g = new CKEDITOR.dom.elementPath(f.getParent(), a.root), d, h = 0, m; h < g.elements.length && (m = g.elements[h]) && m != g.block && m != g.blockLimit; h++) if (this.checkElementRemovable(m)) {
						var k;
					...
					var l = c.endNode,
						n = this;
					b();
					for (g = f; !g.equals(l);) d = g.getNextSourceNode(),
						g.type == CKEDITOR.NODE_ELEMENT && this.checkElementRemovable(g) && (g.getName() == this.element ? q.call(this, g) : w(g, x(this)[g.getName()]), d.type == CKEDITOR.NODE_ELEMENT && d.contains(f) && (b(), d = f.getNext())),
						g = d
				}
				a.moveToBookmark(c);
			...
			function m(a, b) {
				...
				if (e && !g) {
					...
					a.getHtml()
				] : f(a), b) : a.moveChildren(b);
				b.replace(a);
				if (e) {
					var c = b,
						m;
					(m = c.getPrevious(H)) && m.type == CKEDITOR.NODE_ELEMENT && m.is('pre') && (e = n(m.getHtml(), /\n$/, '') + '\n\n' + n(c.getHtml(), /^\n/, ''), CKEDITOR.env.ie ? c.$.outerHTML = '<pre>' + e + '</pre>' : c.setHtml(e), m.remove())
				} else c && t(b)
			}
			function f(a) {
				var b = [
			...
			function q(a, b) {
				var c = this._.definition,
					...
					d;
				for (d in f) if ('class' != d && !this._.definition.fullMatch || a.getAttribute(d) == y(d, f[d])) b && 'data-' == d.slice(0, 5) || (g = a.hasAttribute(d), a.removeAttribute(d));
				for (var h in c) this._.definition.fullMatch && a.getStyle(h) != y(h, c[h], !0) || (g = g || !!a.getStyle(h), a.removeStyle(h));
				w(a, e, A[a.getName()]);
				g && (this._.definition.alwaysRemoveElement ? t(a, 1) : !CKEDITOR.dtd.$block[a.getName()] || this._.enterMode == CKEDITOR.ENTER_BR && !a.hasAttributes() ? t(a) : a.renameNode(this._.enterMode == CKEDITOR.ENTER_P ?
					'p' : 'div'))
			}
			function v(a) {
				for (var b = x(this), c = a.getElementsByTag(this.element), f, e = c.count(); 0 <= --e;) f = c.getItem(e),
					f.isReadOnly() || q.call(this, f, !0);
				for (var g in b) if (g != this.element) for (c = a.getElementsByTag(g), e = c.count() - 1; 0 <= e; e--) f = c.getItem(e),
					f.isReadOnly() || w(f, b[g])
			}
			function w(a, b, c) {
			...
			function t(a, b) {
				if (!a.hasAttributes() || b) if (CKEDITOR.dtd.$block[a.getName()]) {
					...
					f = a.getLast(),
					a.remove(!0),
					c && (c.type == CKEDITOR.NODE_ELEMENT && c.mergeSiblings(), f && !c.equals(f) && f.type == CKEDITOR.NODE_ELEMENT && f.mergeSiblings())
			}
			function u(a, b, c) {
				var f;
				f = a.element;
			...
			function z(a, b, c) {
				...
				var g,
					d;
				if (a.isFake && a.isInTable()) for (g = [
				], d = 0; d < e.length; d++) g.push(e[d].clone());
					...
					if ('string' == typeof this.element ? f == this.element : f in this.element) {
						...
						if (f = c._AC) c = f;
							...
							var f = {
								...
								e = 0,
								g = c.attributes;
							if (g) for (var d in g) e++ ,
								f[d] = g[d];
							if (d = CKEDITOR.style.getStyleText(c)) f.style || e++ ,
								f.style = d;
							f._length = e;
							c = c._AC = f
						}
		...
		function () {
			...
			CKEDITOR.dom.rangeList = function (a) {
				...
				a ? a instanceof CKEDITOR.dom.range && (a = [
					a
				]) : a = [
				];
			...
			var d = {
					...
					return {
						getNextRange: function (k) {
							...
							if (e && 1 < a.length) {
								...
								for (e.moveToBookmark(g.shift()); m--;) h = a[++d],
									h.moveToBookmark(g.shift()),
									e.setEnd(h.endContainer, h.endOffset)
							}
							return e
						}
					}
				...
				createBookmarks: function (b) {
					for (var c = [
					], g, d = 0; d < this.length; d++) {
						c.push(g = this[d].createBookmark(b, !0));
						for (var k = d + 1; k < this.length; k++) this[k] = a(g, this[k]),
							this[k] = a(g, this[k], !0)
					}
					return c
				},
				createBookmarks2: function (a) {
					for (var c = [
					], g = 0; g < this.length; g++) c.push(this[g].createBookmark2(a));
					return c
				},
		...
		CKEDITOR.plugins.add('dialogui', {
			onLoad: function () {
				var a = function (a) {
					...
					for (var b = [
						this._
					], c = 1; c < arguments.length; c++) b.push(arguments[c]);
					b.push(!0);
				...
				CKEDITOR.tools.extend(CKEDITOR.ui.dialog, {
					labeledElement: function (b, c, f, e) {
						if (!(4 > arguments.length)) {
							...
							CKEDITOR.ui.dialog.uiElement.call(this, b, c, f, 'div', null, d, function () {
								...
								'horizontal' != c.labelLayout ? a.push('<label class="cke_dialog_ui_labeled_label' + f + '" ', ' id="' + g.labelId + '"', g.inputId ? ' for="' + g.inputId + '"' : '', (c.labelStyle ? ' style="' + c.labelStyle + '"' : '') + '>', c.label, '</label>', '<div class="cke_dialog_ui_labeled_content"', c.controlStyle ? ' style="' + c.controlStyle + '"' : '', ' role="presentation">', e.call(this, b, c), '</div>') : (f = {
									type: 'hbox',
									widths: c.widths,
									padding: 0,
									children: [
										{
											type: 'html',
											html: '<label class="cke_dialog_ui_labeled_label' + f + '" id="' + g.labelId + '" for="' + g.inputId + '"' + (c.labelStyle ? ' style="' + c.labelStyle + '"' : '') + '>' + CKEDITOR.tools.htmlEncode(c.label) + '</label>'
										},
										{
											type: 'html',
											html: '<span class="cke_dialog_ui_labeled_content"' + (c.controlStyle ? ' style="' + c.controlStyle +
												'"' : '') + '>' + e.call(this, b, c) + '</span>'
										}
									]
						...
						if (!(3 > arguments.length)) {
							...
							CKEDITOR.ui.dialog.labeledElement.call(this, b, c, f, function () {
								...
								var a = [
									...
									b;
								for (b in k) a.push(b + '="' + CKEDITOR.tools.htmlEncode(k[b]) + '" ');
					...
					checkbox: function (b, c, f) {
						if (!(3 > arguments.length)) {
							...
							CKEDITOR.ui.dialog.uiElement.call(this, b, c, f, 'span', null, null, function () {
								...
								f.push(' <label id="', g, '" for="', d.id, '"' + (c.labelStyle ? ' style="' + c.labelStyle + '"' : '') + '>', CKEDITOR.tools.htmlEncode(c.label), '</label>');
								return f.join('')
							})
						}
					},
					radio: function (b, c, f) {
						if (!(3 > arguments.length)) {
							...
							CKEDITOR.ui.dialog.labeledElement.call(this, b, c, f, function () {
								for (var a = [
								...
								], d = (c.id ? c.id : CKEDITOR.tools.getNextId()) + '_radio', l = 0; l < c.items.length; l++) {
									var u = c.items[l],
									...
									k(x);
									k(r);
									'undefined' != typeof x.inputStyle && (x.style = x.inputStyle);
					...
					button: function (b, c, f) {
						if (arguments.length) {
							...
							CKEDITOR.ui.dialog.uiElement.call(this, b, g, f, 'a', null, {
								...
								role: 'button',
								'aria-labelledby': d
							}, '<span id="' + d + '" class="cke_dialog_ui_button">' +
							CKEDITOR.tools.htmlEncode(c.label) + '</span>')
						}
					},
					select: function (b, c, f) {
						if (!(3 > arguments.length)) {
							...
							CKEDITOR.ui.dialog.labeledElement.call(this, b, c, f, function () {
								...
								f.push('<div class="cke_dialog_ui_input_', c.type, '" role="presentation"');
								c.width && f.push('style="width:' + c.width + '" ');
								f.push('>');
								void 0 !== c.size && (d.size = c.size);
								void 0 !== c.multiple && (d.multiple = c.multiple);
								k(a);
								for (var l = 0, u; l < c.items.length && (u = c.items[l]); l++) g.push('<option value="', CKEDITOR.tools.htmlEncode(void 0 !== u[1] ? u[1] : u[0]).replace(/"/g, '&quot;'), '" /> ', CKEDITOR.tools.htmlEncode(u[0]));
								'undefined' != typeof a.inputStyle && (a.style = a.inputStyle);
								e.select = new CKEDITOR.ui.dialog.uiElement(b, a, f, 'select', null, d, g.join(''));
								f.push('</div>');
								return f.join('')
							})
						}
					...
					fieldset: function (a, b, c, e, g) {
						...
						CKEDITOR.ui.dialog.uiElement.call(this, a, g, e, 'fieldset', null, null, function () {
							...
							d && a.push('<legend' + (g.labelStyle ? ' style="' + g.labelStyle + '"' : '') + '>' + d + '</legend>');
				...
				CKEDITOR.ui.dialog.textInput.prototype = CKEDITOR.tools.extend(new CKEDITOR.ui.dialog.labeledElement, {
					...
					setDirectionMarker: function (a) {
						...
						}) : this.getDirectionMarker() && b.removeAttributes(['dir',
							'data-cke-dir-marker'])
					},
				...
				CKEDITOR.ui.dialog.radio.prototype = CKEDITOR.tools.extend(new CKEDITOR.ui.dialog.uiElement, {
					setValue: function (a, b) {
						for (var c = this._.children, e, g = 0; g < c.length && (e = c[g]); g++) e.getElement().$.checked = e.getValue() == a;
						!b && this.fire('change', {
							value: a
						})
					},
				...
				CKEDITOR.ui.dialog.file.prototype = CKEDITOR.tools.extend(new CKEDITOR.ui.dialog.labeledElement, c, {
					...
					reset: function () {
						function a() {
							...
							c.$.write(['<html dir="' + l + '" lang="' + t + '"><head><title></title></head><body style="margin: 0; overflow: hidden; background: transparent;">',
							'<form enctype="multipart/form-data" method="POST" dir="' + l + '" lang="' + t + '" action="',
							...
							'window.parent.CKEDITOR.tools.callFunction(' + d + ');',
				...
				CKEDITOR.dialog.addUIElement('fieldset', {
					build: function (a, b, c) {
						for (var e = b.children, g, d = [
						], k = [
						], l = 0; l < e.length && (g = e[l]); l++) {
							var t = [
							];
							d.push(t);
							k.push(CKEDITOR.dialog._.uiElementBuilders[g.type].build(a, g, t))
						}
		...
		function () {
			...
			function m(a) {
				...
				a.parts.title.on('mousedown', function (g) {
					...
					e = a.getPosition();
					if (CKEDITOR.env.ie6Compat) {
						var d = z.getChild(0).getFrameDocument();
						d.on('mousemove', b);
						d.on('mouseup', c)
					}
			...
			function f(a) {
				function b(c) {
					...
					if (e == CKEDITOR.DIALOG_RESIZE_WIDTH || e == CKEDITOR.DIALOG_RESIZE_BOTH) r = Math.max(f.minWidth || 0, p - d);
				...
				if (e != CKEDITOR.DIALOG_RESIZE_NONE) {
					var g = a.getParentEditor(),
						...
						r = CKEDITOR.tools.addFunction(function (f) {
							...
							e.$.getElementsByTagName('iframe').length && (n = CKEDITOR.dom.element.createFromHtml('<div class="cke_dialog_resize_cover" style="height: 100%; position: absolute; width: 100%;"></div>'), e.append(n));
					...
					a.on('load', function () {
						...
						b = CKEDITOR.dom.element.createFromHtml('<div class="cke_resizer' + b + ' cke_resizer_' + g.lang.dir + '" title="' + CKEDITOR.tools.htmlEncode(g.lang.common.resize) +
							'" onmousedown="CKEDITOR.tools.callFunction(' + r + ', event )">' + ('ltr' == g.lang.dir ? '◢' : '◣') + '</div>');
			...
			function p(a) {
				...
				], CKEDITOR.env.ie6Compat && (e = '<html><body style=\\\'background-color:' + e + ';\\\'></body></html>', g.push('<iframe hidefocus="true" frameborder="0" id="cke_dialog_background_iframe" src="javascript:'), g.push('void((function(){' + encodeURIComponent('document.open();(' +
					CKEDITOR.tools.fixDomain + ')();document.write( \'' + e + '\' );document.close();') + '})())'), g.push('" style="position:absolute;left:0;top:0;width:100%;height: 100%;filter: progid:DXImageTransform.Microsoft.Alpha(opacity=0)"></iframe>')), g.push('</div>'), d = CKEDITOR.dom.element.createFromHtml(g.join('')), d.setOpacity(void 0 !== f ? f : 0.5), d.on('keydown', n), d.on('keypress', n), d.on('keyup', n), d.appendTo(CKEDITOR.document.getBody()), C[c] = d);
			...
			var v = CKEDITOR.tools.cssLength,
				w = '<div class="cke_reset_all {editorId} {editorDialogClass} {hidpi}" dir="{langDir}" lang="{langCode}" role="dialog" aria-labelledby="cke_dialog_title_{id}"><table class="cke_dialog ' +
			...
			CKEDITOR.dialog = function (b, e) {
				...
				function n(c) {
					if (A == CKEDITOR.dialog._.currentTop) {
						...
						B = w = 0;
						if (9 == f || f == CKEDITOR.SHIFT + 9) k(f == CKEDITOR.SHIFT + 9 ? - 1 : 1),
							B = 1;
						else if (f == CKEDITOR.ALT + 121 && !A._.tabBarMode && 1 < A.getPageCount()) A._.tabBarMode = !0,
							A._.tabs[A._.currentTabId][0].focus(),
							A._.currentFocusIndex = - 1,
							B = 1;
						else if (- 1 != CKEDITOR.tools.indexOf(g, f) && A._.tabBarMode) f = - 1 != CKEDITOR.tools.indexOf([e ? 39 : 37,
							38], f) ? a.call(A) : d.call(A),
							A.selectPage(f),
							A._.tabs[f][0].focus(),
							B = 1;
						else if (13 != f && 32 != f || !A._.tabBarMode) if (13 == f) f = c.data.getTarget(),
							f.is('a', 'button', 'select', 'textarea') || f.is('input') && 'button' == f.$.type || ((f = this.getButton('ok')) && CKEDITOR.tools.setTimeout(f.click, 0, f), B = 1),
							w = 1;
				...
				function r(a) {
					B ? a.data.preventDefault(1) : w && a.data.stopPropagation()
				}
				...
				if (b._.removeDialogTabs && (y = b._.removeDialogTabs[e])) for (q = 0; q < y.length; q++) p.removeContents(y[q]);
				...
				this.on('show', function () {
					C.on('keydown', n, this);
					if (CKEDITOR.env.gecko) C.on('keypress', r, this)
				});
				...
				this.on('show', function () {
					...
					b.config.dialog_startupFocusTab && a ? (A._.tabBarMode = !0, A._.tabs[A._.currentTabId][0].focus(), A._.currentFocusIndex = - 1) : this._.hasFocus || (this._.currentFocusIndex = a ? - 1 : this._.focusList.length - 1, p.onFocus ? (a = p.onFocus.call(this)) && a.focus() : k(1))
			...
			CKEDITOR.dialog.prototype = {
				...
				addPage: function (a) {
					if (!a.requiredContent || this._.editor.filter.check(a.requiredContent)) {
						for (var b = [
						], c = a.label ? ' title="' + CKEDITOR.tools.htmlEncode(a.label) + '"' : '', f = CKEDITOR.dialog._.uiElementBuilders.vbox.build(this, {
							type: 'vbox',
						...
						}, g = f.getChild(), d = 0; f = g.shift();) f.notAllowed || 'hbox' == f.type || 'vbox' == f.type || d++ ,
							e[f.id] = f,
							'function' == typeof f.getChild && g.push.apply(g, f.getChild());
						d || (a.hidden = !0);
						...
						c = CKEDITOR.dom.element.createFromHtml(['<a class="cke_dialog_tab"',
							...
							e,
							'"',
							f.gecko && !f.hc ? '' : ' href="javascript:void(0)"',
			...
			var t = {
				...
				r = function (a, b, c, f, e) {
					if (c) {
						for (var g = 0, d; d = a[g]; g++) {
							if (d.id == c) return a.splice(g, 0, b),
								b;
							if (f && d[f] && (d = r(d[f], b, c, f, !0))) return d
						}
						if (e) return null
					}
					a.push(b);
			...
			h.prototype = {
				get: function (a) {
					return u(this.elements, a, 'children')
				},
				add: function (a, b) {
			...
			var y,
				...
				I = function (a) {
					for (var b in A) {
						for (var c = A[b], f = c.length - 1; 0 <= f; f--) c[f].dialog != a && c[f].uiElement != a || c.splice(f, 1);
				...
				CKEDITOR.ui.dialog = {
					uiElement: function (a, b, c, f, e, g, d) {
						if (!(4 > arguments.length)) {
							...
							for (var t = l['class'] && l['class'].split ? l['class'].split(' ') : [
							], r = 0; r < t.length; r++) t[r] && (p[t[r]] = 1);
							t = [
							];
							for (r in p) t.push(r);
							l['class'] = t.join(' ');
							...
							0 < p.length && (l.style = (l.style ? l.style + '; ' : '') + p.join('; '));
							for (r in l) k.push(r + '="' + CKEDITOR.tools.htmlEncode(l[r]) + '" ');
							k.push('>', n, '</', h, '>');
							c.push(k.join(''));
							(this._ || (this._ = {
							})).dialog = a;
							...
							CKEDITOR.ui.dialog.uiElement.call(this, a, e || {
								type: 'hbox'
									...
									for (k = 0; k < c.length; k++) {
										...
										CKEDITOR.env.ie && CKEDITOR.env.quirks && g[k].align && f.push('text-align:' + g[k].align);
										0 < f.length && a.push('style="' + f.join('; ') + '" ');
										a.push('>', c[k], '</td>')
									}
									a.push('</tr></tbody>');
									return a.join('')
			...
			(function () {
				var a = {
					build: function (a, b, c) {
						for (var f = b.children, e, g = [
						], d = [
						], h = 0; h < f.length && (e = f[h]); h++) {
							var k = [
							];
							g.push(k);
				...
				CKEDITOR.dialog.validate = {
					functions: function () {
						...
						return function () {
							...
							for (g = 0; g < e.length; g++) d = f == CKEDITOR.VALIDATE_AND ? d && e[g](b) : d || e[g](b);
							return d ? !0 : c
						}
					},
					regex: function (a, b) {
			...
			CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
				openDialog: function (a, b) {
					...
					if ('function' == typeof f) c = this._.storedDialogs || (this._.storedDialogs = {
					}),
						c = c[a] || (c[a] = new CKEDITOR.dialog(this, a)),
						b && b.call(c, c),
						c.show();
					else {
						if ('failed' == f) throw q(this),
		...
		CKEDITOR.plugins.add('basicstyles', {
			init: function (a) {
				var d = 0,
					b = function (b, e, g, m) {
						if (m) {
							...
							a.addCommand(g, new CKEDITOR.styleCommand(m, {
								contentForms: f
							}));
				...
				a.setKeystroke([[CKEDITOR.CTRL + 66,
					'bold'],
				[
					CKEDITOR.CTRL + 73,
					'italic'
		...
		function () {
			...
			function d(a, b, c) {
				...
				if (a.checkBoundaryOfElement(e, CKEDITOR.START) && a.checkBoundaryOfElement(e, CKEDITOR.END)) {
					for (var g; e && e.type == CKEDITOR.NODE_ELEMENT && (g = e.getParent()) && 1 == g.getChildCount() && !(e.getName() in b);) e = g;
			...
			function b(b) {
				return {
					...
					exec: function (c) {
						...
						if (k && k.length) {
							for (var m = {
							}, t = e.createBookmarks(), k = k.createIterator(), u, r = 0; u = k.getNextRange(1);) {
			...
			function c(a) {
				...
				return function (e, d) {
					if (!this.isReadOnly()) {
						...
						if (h && (h = this.getDirection(1), k = a.apply(this, arguments), h != this.getDirection(1))) return this.getDocument().fire('dirChanged', this),
							k
					}
			...
			for (var e = CKEDITOR.dom.element.prototype, h = [
				'setStyle',
				'removeStyle',
				'setAttribute',
				'removeAttribute'
			], m = 0; m < h.length; m++) e[h[m]] = CKEDITOR.tools.override(e[h[m]], c)
		}(),
		function () {
			var a = {
				exec: function (a) {
					...
					if (g) {
						...
						if (b == CKEDITOR.TRISTATE_OFF) {
							...
							for (; 0 < g.length;) if (b = g.shift(), 'blockquote' == b.getName()) {
								for (e = new CKEDITOR.dom.documentFragment(a.document); b.getFirst();) e.append(b.getFirst().remove()),
									k.push(e.getLast());
								e.replace(b)
							} else k.push(b);
							g = a.document.createElement('blockquote');
							for (g.insertBefore(k[0]); 0 < k.length;) b = k.shift(),
								g.append(b)
						} else if (b == CKEDITOR.TRISTATE_ON) {
							e = [
							];
							for (h = {
							}; b = m.getNextParagraph();) {
								...
								g && k && !k.getCustomData('blockquote_moveout') && (e.push(k), CKEDITOR.dom.element.setMarker(h, k, 'blockquote_moveout', !0))
							}
							...
							for (h = {
								...
								g = m.getParent(),
								m.getPrevious() ? m.getNext() ? (m.breakParent(m.getParent()), k.push(m.getNext())) : m.remove().insertAfter(g) : m.remove().insertBefore(g),
								g.getCustomData('blockquote_processed') || (k.push(g), CKEDITOR.dom.element.setMarker(h, g, 'blockquote_processed', !0)),
								b.push(m);
							CKEDITOR.dom.element.clearAllMarkers(h);
							...
							if (a.config.enterMode == CKEDITOR.ENTER_BR) for (g = !0; b.length;) if (m = b.shift(), 'div' == m.getName()) {
								...
								for (g = m.getNext() && !(m.getNext().type == CKEDITOR.NODE_ELEMENT && m.getNext().isBlockBoundary()); m.getFirst();) m.getFirst().remove().appendTo(e);
		...
		function () {
			...
			a.prototype = {
				...
				update: function (a) {
					...
					e && (g.removeClass(this._getClass()), g.removeAttribute('aria-label'), this.type = e, g.addClass(this._getClass()), g.setAttribute('aria-label', this.type), 'progress' != this.type || k ? 'progress' != this.type && k && k.remove() : (k = this._createProgressElement(), k.insertBefore(d)));
				...
				_createElement: function () {
					...
					c.append(g);
					g = CKEDITOR.dom.element.createFromHtml('<a class="cke_notification_close" href="javascript:void(0)" title="' +
						d + '" role="button" tabindex="-1"><span class="cke_label">X</span></a>');
					c.append(g);
					g.on('click', function () {
						a.editor.focus();
						a.hide()
				...
				_hideAfterTimeout: function () {
					...
					else if ('info' == this.type || 'success' == this.type) c = 'number' == typeof this.editor.config.notification_duration ? this.editor.config.notification_duration : 5000;
			...
			d.prototype = {
				...
				_layout: function () {
					function a() {
						c.setStyle('left', u(r + d.width - n - p))
					}
					var c = this.element,
						g = this.editor,
					...
					var q = f.getScrollPosition(),
						v = f.getViewPaneSize(),
						w = CKEDITOR.document.getBody(),
						t = w.getDocumentPosition(),
					...
					e && e.isVisible() && h.bottom > d.top && h.bottom < d.bottom - m.height ? c.setStyles({
						...
						position: 'absolute',
						top: u(k.y)
					}) : k.y + d.height - m.height > q.y ? c.setStyles({
						position: 'fixed',
						top: 0
					}) : c.setStyles({
						position: 'absolute',
					...
					var r = 'fixed' == c.getStyle('position') ? d.left : 'static' != w.getComputedStyle('position') ? k.x - t.x : k.x;
					d.width < n + p ? k.x + n + p > q.x + v.width ? a() : c.setStyle('left', u(r)) : k.x + n + p > q.x + v.width ? c.setStyle('left', u(r)) : k.x + d.width / 2 + n / 2 + p > q.x + v.width ? c.setStyle('left', u(r - k.x + q.x + v.width - n - p)) : 0 > d.left + d.width - n - p ? a() : 0 > d.left + d.width / 2 - n / 2 ? c.setStyle('left', u(r - k.x + q.x)) : c.setStyle('left', u(r + d.width / 2 - n / 2 - p / 2))
				}
			};
			CKEDITOR.plugins.notification = a
		}(),
		function () {
			var a = '<a id="{id}" class="cke_button cke_button__{name} cke_button_{state} {cls}"' + (CKEDITOR.env.gecko && !CKEDITOR.env.hc ? '' : ' href="javascript:void(\'{titleJs}\')"') + ' title="{title}" tabindex="-1" hidefocus="true" role="button" aria-labelledby="{id}_label" aria-describedby="{id}_description" aria-haspopup="{hasArrow}" aria-disabled="{ariaDisabled}"';
			CKEDITOR.env.gecko && CKEDITOR.env.mac && (a += ' onkeypress="return false;"');
			...
			var a = a + (' onkeydown="return CKEDITOR.tools.callFunction({keydownFn},event);" onfocus="return CKEDITOR.tools.callFunction({focusFn},event);" ' + (CKEDITOR.env.ie ? 'onclick="return false;" onmouseup' : 'onclick') + '="CKEDITOR.tools.callFunction({clickFn},this);return false;"><span class="cke_button_icon cke_button__{iconName}_icon" style="{style}"'),
				a = a + '>&nbsp;</span><span id="{id}_label" class="cke_button_label cke_button__{name}_label" aria-hidden="false">{label}</span><span id="{id}_description" class="cke_button_label" aria-hidden="false">{ariaShortcut}</span>{arrowHtml}</a>',
				...
				render: function (a, g) {
					...
					var v = {
						...
						w = CKEDITOR.tools.addFunction(function (a) {
							if (v.onkey) return a = new CKEDITOR.dom.event(a),
			...
			function a(a) {
				function b() {
					for (var f = c(), h = CKEDITOR.tools.clone(a.config.toolbarGroups) || d(a), m = 0; m < h.length; m++) {
						...
						if ('/' != l) {
							...
							var w,
								t = l.groups;
							if (t) for (var u = 0; u < t.length; u++) w = t[u],
								(w = f[w]) && e(l, w);
							(w = f[l.name]) && e(l, w)
						}
					}
				...
				function c() {
					var b = {
						...
						e,
						d;
					for (f in a.ui.items) e = a.ui.items[f],
						d = e.toolbar || 'others',
						d = d.split(','),
						e = d[0],
						d = parseInt(d[1] || - 1, 10),
				...
				function e(b, c) {
					if (c.length) {
						...
						for (var f; f = c.shift();) f = 'string' == typeof f ? f : f.name,
							m && - 1 != CKEDITOR.tools.indexOf(m, f) || (f = a.ui.create(f)) && a.addFeature(f) && b.items.push(f)
					}
				}
				function h(a) {
					var b = [
					...
					for (c = 0; c < a.length; ++c) f = a[c],
						...
						'/' == f ? b.push(f) : CKEDITOR.tools.isArray(f) ? (e(g, CKEDITOR.tools.clone(f)), b.push(g)) : f.items && (e(g, CKEDITOR.tools.clone(f.items)), g.name = f.name, b.push(g));
					return b
				}
						...
						k = function (a, b) {
							...
							switch (b) {
								...
								case CKEDITOR.SHIFT + 9:
									for (; !c || !c.items.length;) if (c = 9 == b ? (c ? c.next : a.toolbar.next) || g.toolbox.toolbars[0] : (c ? c.previous : a.toolbar.previous) || g.toolbox.toolbars[g.toolbox.toolbars.length - 1], c.items.length) for (a = c.items[d ? c.items.length - 1 : 0]; a && !a.focus;) (a = d ? a.previous : a.next) || (c = 0);
									a && a.focus();
									return !1;
								case p:
									c = a;
									...
									return !1;
								case 40:
									return a.button && a.button.hasArrow ? a.execute() : k(a, 40 == b ? p : f),
										!1;
								case f:
								case 38:
									c = a;
									do c = c.previous,
										!c && n && (c = a.toolbar.items[a.toolbar.items.length -
											1]);
									while (c && !c.focus);
						...
						if (c.data.space == g.config.toolbarLocation) {
							...
							var d = CKEDITOR.tools.getNextId(),
								m = [
									...
									'</span>',
									'<span id="' + g.ui.spaceId('toolbox') + '" class="cke_toolbox" role="group" aria-labelledby="',
									d,
								...
								f,
								l;
							g.config.toolbarCanCollapse && g.elementMode != CKEDITOR.ELEMENT_MODE_INLINE && m.push('<span class="cke_toolbox_main"' + (d ? '>' : ' style="display:none">'));
							for (var p = g.toolbox.toolbars, q = a(g), v = q.length, w = 0; w < v; w++) {
								var t,
									...
									x = '/' !== B && ('/' === q[w + 1] || w == v - 1),
									y;
								if (B) if (f && (m.push('</span>'), l = f = 0), '/' === B) m.push('<span class="cke_toolbar_break"></span>');
								else {
									y = B.items || B;
										...
										if (z) {
											...
											if (z.type == CKEDITOR.UI_SEPARATOR) l = f && z;
												...
												if (!u) {
													...
													m.push('<span id="', t, '" class="cke_toolbar' + (x ? ' cke_toolbar_last"' : '"'), r ? ' aria-labelledby="' + t + '_label"' : '', ' role="toolbar">');
													r && m.push('<span id="', t, '_label" class="cke_voice_label">', r, '</span>');
													m.push('<span class="cke_toolbar_start"></span>');
													var D = p.push(u) - 1;
													0 < D && (u.previous = p[D - 1], u.previous.next = u)
												}
												A ? f || (m.push('<span class="cke_toolgroup" role="presentation">'), f = 1) : f && (m.push('</span>'), f = 0);
												l && (G(l), l = 0);
												G(z)
											}
										}
									...
									f && (m.push('</span>'), l = f = 0);
									u && m.push('<span class="cke_toolbar_end"></span></span>')
								}
							}
							g.config.toolbarCanCollapse && m.push('</span>');
							if (g.config.toolbarCanCollapse && g.elementMode != CKEDITOR.ELEMENT_MODE_INLINE) {
								...
								m.push('<a title="' + (d ? g.lang.toolbar.toolbarCollapse : g.lang.toolbar.toolbarExpand) + '" id="' + g.ui.spaceId('toolbar_collapser') + '" tabIndex="-1" class="cke_toolbox_collapser');
								d || m.push(' cke_toolbox_collapser_min');
								m.push('" onclick="CKEDITOR.tools.callFunction(' + F + ')">', '<span class="cke_arrow">&#9650;</span>', '</a>')
							}
							m.push('</span>');
							c.data.html += m.join('')
						}
					...
					g.on('destroy', function () {
						if (this.toolbox) {
							var a,
								...
								f,
								g;
							for (a = this.toolbox.toolbars; b < a.length; b++) for (f = a[b].items, c = 0; c < f.length; c++) g = f[c],
		...
		function () {
			...
			function d(b) {
				function c() {
					...
					a.on(A.mainPasteEvent, function (a) {
						'beforepaste' == A.mainPasteEvent && G || y(a)
					});
					'beforepaste' == A.mainPasteEvent && (a.on('paste', function (a) {
						D || (d(), a.data.preventDefault(), y(a), k('paste'))
					...
					return {
						...
						exec: function (b, c) {
							...
							var e = this,
								...
								k = 'string' === typeof g ? g : b.lang.clipboard.pasteNotification.replace(/%1/, '<kbd aria-label="' + h.aria + '">' + h.display + '</kbd>'),
				...
				function m() {
					if (CKEDITOR.env.ie && !CKEDITOR.env.quirks) {
						var a = b.getSelection(),
							...
							e,
							g;
						a.getType() == CKEDITOR.SELECTION_ELEMENT && (c = a.getSelectedElement()) && (e = a.getRanges()[0], g = b.document.createText(''), g.insertBefore(c), e.setStartBefore(g), e.setEndAfter(c), a.selectRanges([e]), setTimeout(function () {
				...
				function l(a, c) {
					...
					if (!e.getById('cke_pastebin')) {
						...
						var r = 0,
							e = e.getWindow();
						CKEDITOR.env.webkit ? (g.append(n), n.addClass('cke_editable'), g.is('body') || (r = 'static' != g.getComputedStyle('position') ? g : CKEDITOR.dom.element.get(g.$.offsetParent), r = r.getDocumentPosition().y)) : g.getAscendant(CKEDITOR.env.ie ? 'body' : 'html', 1).append(n);
						n.setStyles({
							position: 'absolute',
							top: e.getScrollPosition().y - r + 10 + 'px',
							width: '1px',
				...
				function B() {
					...
					a.lock();
					if (b.editable().fire(A.mainPasteEvent) && !k('paste')) return a.unlock(),
						!1;
					a.unlock();
					return !0
				}
				...
				function y(c) {
					...
					g && A.canClipboardApiBeTrusted(e.dataTransfer, b) ? (c.data.preventDefault(), setTimeout(function () {
						a(b, e)
					}, 0)) : l(c, function (c) {
						e.dataValue = c.replace(/<span[^>]+data-cke-bookmark[^<]*?<\/span>/gi, '');
						g && a(b, e)
					})
				}
				function C() {
				...
				b.getClipboardData = function (a, c) {
					...
					!1 === B() && (b.removeListener('paste', e), b._.forcePasteDialog && d && b.fire('pasteDialog') ? (b.on('pasteDialogCommit', g), b.on('dialogHide', function (a) {
						a.removeListener();
				...
				init: function (a) {
						...
						a.on('paste', function (b) {
							...
							if (!c.dataValue && 'paste' == c.method && e && 1 == e.getFilesCount() && v != e.id && (e = e.getFile(0), - 1 != CKEDITOR.tools.indexOf(m, e.type))) {
								...
								g.addEventListener('load', function () {
									b.data.dataValue = '<img src="' + g.result + '" />';
									a.fire('paste', b.data)
								}, !1);
					...
					a.on('paste', function (b) {
						...
						if (!b.data.dataValue) {
							...
							if (e) b.data.dataValue = e,
								b.data.type = 'html';
							else if (e = c.getData('text/plain')) b.data.dataValue = a.editable().transformPlainTextToHtml(e),
								b.data.type = 'text'
						}
					}, null, null, 1);
					a.on('paste', function (a) {
						...
						CKEDITOR.env.ie ? b = b.replace(/^&nbsp;(?: |\r\n)?<(\w+)/g, function (b, e) {
							return e.toLowerCase() in c ? (a.data.preSniffing = 'html', '<' + e) : b
						}) : CKEDITOR.env.webkit ? b = b.replace(/<\/(\w+)><div><br><\/div>$/, function (b, e) {
							return e in c ? (a.data.endsWithEOL = 1, '</' + e + '>') : b
					...
					a.on('paste', function (e) {
						...
						else if (n && a.pasteFilter && !e.dontFilter || v) d = l(a, d, a.pasteFilter);
						e.startsWithEOL && (d = '<br data-cke-eol="1">' + d);
						e.endsWithEOL && (d += '<br data-cke-eol="1">');
			...
			CKEDITOR.plugins.clipboard = {
				...
				addPasteButton: function (a, b, c) {
					a.ui.addButton && (a.ui.addButton(b, c), a._.pasteButtons || (a._.pasteButtons = [
				...
				fixSplitNodesAfterDrop: function (a, b, c, e) {
					function g(a, c, e) {
						...
						if (f.equals(c) && e != c.getChildCount()) return a = b.startContainer.getChild(b.startOffset - 1),
							c = b.startContainer.getChild(b.startOffset),
							a && a.type == CKEDITOR.NODE_TEXT && c && c.type == CKEDITOR.NODE_TEXT && (e = a.getLength(), a.setText(a.getText() + c.getText()), c.remove(), b.setStart(a, e), b.collapse(!0)),
							!0
					}
				...
				getRangeAtDropPosition: function (a, b) {
					...
					if (document.caretRangeFromPoint && b.document.$.caretRangeFromPoint(e, g)) c = b.document.$.caretRangeFromPoint(e, g),
							...
							try {
								...
								if (k) {
									...
									var z = b.document.$.elementFromPoint(e, g),
										...
										G;
									if (A.equals(b.editable()) || 'html' == A.getName()) return d && d.startContainer && !d.startContainer.equals(b.editable()) ? d : null;
									G = A.getClientRect();
			...
			CKEDITOR.plugins.clipboard.dataTransfer.prototype = {
				...
				setData: function (a, b) {
					...
					if (CKEDITOR.plugins.clipboard.isCustomDataTypesSupported || 'URL' == a || 'Text' == a) if ('Text' == m && 'Text' == a && (this.id = b), this._.fallbackDataTransfer.isRequired()) this._.fallbackDataTransfer.setData(a, b);
					else try {
						this.$.setData(a, b)
					} catch (c) {
					}
			...
			CKEDITOR.plugins.clipboard.fallbackDataTransfer.prototype = {
					...
					if (null === a._isCustomMimeTypeSupported) if (b) {
						...
						try {
							b.setData('cke/mimetypetest', 'cke test value'),
								a._isCustomMimeTypeSupported = 'cke test value' === b.getData('cke/mimetypetest'),
				...
				setData: function (a, b) {
					...
					try {
						g.setData(a, e),
							c && (this._dataTransfer._.nativeHtmlCache = e)
					} catch (d) {
				...
				_applyDataComment: function (a, b) {
					var c = '';
					b && CKEDITOR.tools.objectKeys(b).length && (c = '<!--cke-data:' + encodeURIComponent(JSON.stringify(b)) + '-->');
				...
				proto: {
					...
					onKeyDown: function (a, b) {
						...
						switch (d) {
							...
							case 'click':
							case 'mouseup':
								return k = this._.focusIndex,
									(e = 0 <= k && this.element.getElementsByTag('a').getItem(k)) && (e.$[d] ? e.$[d]() : e.$['on' + d]()),
									!1
						}
						return !0
					}
				...
				proto: {
					...
					showBlock: function (a, c, g, d, k, e) {
						...
						var n = this.element,
							...
							t = v.x + (d || 0) - p.x,
							u = v.y + (k || 0) - p.y;
						!w || 1 != g && 4 != g ? w || 2 != g && 3 != g || (t += c.$.offsetWidth - 1) : t += c.$.offsetWidth;
						...
						CKEDITOR.tools.setTimeout(function () {
							var a = CKEDITOR.tools.bind(function () {
								...
								var b = h.element.getWindow(),
									...
									g = w ? a.right : b.width - a.left,
									d = w ? b.width - a.right : a.left;
								w ? g < c && (t = d > c ? t + c : b.width > c ? t - a.left : t - a.right + b.width) : g < c && (t = d > c ? t - c : b.width > c ? t - a.right + b.width : t - a.left);
								c = a.top;
								b.height -
					...
					showAsChild: function (a, c, g, d, k, e) {
						if (this._.activeChild != a || a._.panel._.offsetParentId != g.getId()) this.hideChild(),
		...
		CKEDITOR.plugins.add('colorbutton', {
			...
			init: function (a) {
				function d(c, e, d, h, q) {
					var v = new CKEDITOR.style(l['colorButton_' + e + 'Style']),
						w = CKEDITOR.tools.getNextId() + '_colorBox',
						t;
					q = q || {
					};
					a.ui.add(c, CKEDITOR.UI_PANELBUTTON, {
						...
						onOpen: function () {
							...
							if (d) {
								...
								do d = c && c.getComputedStyle('back' == e ? 'background-color' : 'color') || 'transparent';
								while ('back' == e && 'transparent' == d && c && (c = c.getParent()));
								d && 'transparent' != d || (d = '#ffffff');
								!1 !== l.colorButton_enableAutomatic && this._.panel._.iframe.getFrameDocument().getById(w).setStyle('background-color', d);
				...
				function b(b, e, d) {
					...
					var g = l.colorButton_colors.split(','),
						...
						v = a.plugins.colordialog && !1 !== l.colorButton_enableMore,
						w = g.length + (v ? 2 : 1),
						t = CKEDITOR.tools.addFunction(function (b, e) {
							function f(b) {
								a.removeStyle(new CKEDITOR.style(l['colorButton_' + e + 'Style'], {
									color: 'inherit'
								}));
					...
					!1 !== l.colorButton_enableAutomatic && b.push('<a class="cke_colorauto" _cke_focus=1 hidefocus=true title="', k.auto, '" onclick="CKEDITOR.tools.callFunction(', t, ',null,\'', e, '\');return false;" href="javascript:void(\'', k.auto, '\')" role="option" aria-posinset="1" aria-setsize="', w, '"><table role="presentation" cellspacing=0 cellpadding=0 width="100%"><tr><td colspan="' +
						h + '" align="center"><span class="cke_colorbox" id="', d, '"></span>', k.auto, '</td></tr></table></a>');
					b.push('<table role="presentation" cellspacing=0 cellpadding=0 width="100%">');
					for (d = 0; d < g.length; d++) {
						...
						b.push('<td><a class="cke_colorbox" _cke_focus=1 hidefocus=true title="', u, '" onclick="CKEDITOR.tools.callFunction(', t, ',\'', r, '\',\'', e, '\'); return false;" href="javascript:void(\'', u, '\')" data-value="' + B + '" role="option" aria-posinset="', d + 2, '" aria-setsize="', w, '"><span class="cke_colorbox" style="background-color:#', B, '"></span></a></td>')
					}
					v && b.push('</tr><tr><td colspan="' + h + '" align="center"><a class="cke_colormore" _cke_focus=1 hidefocus=true title="', k.more, '" onclick="CKEDITOR.tools.callFunction(', t, ',\'?\',\'', e, '\');return false;" href="javascript:void(\'', k.more, '\')"', ' role="option" aria-posinset="', w, '" aria-setsize="', w, '">', k.more, '</a></td>');
					b.push('</tr></table>');
					return b.join('')
				}
				function c(a) {
				...
				if (!CKEDITOR.env.hc) {
					d('TextColor', 'fore', k.textColorTitle, 10, {
		...
		function () {
					...
					a.copyFormatting.on('applyFormatting', function (g) {
						if (!g.data.preventFormatStripping) {
							var m = g.data.range,
								...
								p,
								q;
							if (a.copyFormatting._isContextAllowed(l)) for (q = 0; q < f.length; q++) l = f[q],
								p = m.createBookmark(),
								- 1 === c(b.preservedElements, l.element) ? CKEDITOR.env.webkit && !CKEDITOR.env.chrome ? f[q].removeFromRange(g.data.range, g.editor) : f[q].remove(g.editor) : d(l.element, g.data.styles) && b._removeStylesFromElementInRange(m, l.element),
								m.moveToBookmark(p)
						}
					});
			...
			CKEDITOR.plugins.copyformatting = {
				...
				commands: {
					copyFormatting: {
						exec: function (a, b) {
							...
							if (this.state === CKEDITOR.TRISTATE_ON) return d.styles = null,
								d.sticky = !1,
								l.removeClass('cke_copyformatting_active'),
								q.removeClass('cke_copyformatting_disabled'),
								q.removeClass('cke_copyformatting_tableresize_cursor'),
								c._putScreenReaderMessage(a, 'canceled'),
					...
					applyFormatting: {
						...
						exec: function (a, b) {
							...
							if (d || c.state === CKEDITOR.TRISTATE_ON) {
								if (d && !g.styles) return f._putScreenReaderMessage(a, 'failed'),
									f._detachPasteKeystrokeHandler(a),
									!1;
				...
				_extractStylesFromElement: function (a, b) {
					...
					do if (b.type === CKEDITOR.NODE_ELEMENT && !b.hasAttribute('data-cke-bookmark') && (d.element = b, a.copyFormatting.fire('extractFormatting', d, a) && d.styleDef && g.push(new CKEDITOR.style(d.styleDef)), b.getName && - 1 !== c(CKEDITOR.plugins.copyformatting.breakOnElements, b.getName()))) break;
					while ((b = b.getParent()) && b.type === CKEDITOR.NODE_ELEMENT);
					return g
				},
				_extractStylesFromRange: function (a, b) {
					for (var c = [
					], d = new CKEDITOR.dom.walker(b), f; f = d.next();) c = c.concat(CKEDITOR.plugins.copyformatting._extractStylesFromElement(a, f));
					return c
				},
				_removeStylesFromElementInRange: function (a, b) {
					for (var d = - 1 !== c(['ol',
						'ul',
						'table'], b), g = new CKEDITOR.dom.walker(a), f; f = g.next();) if (f = f.getAscendant(b, !0)) if (f.removeAttributes(f.getAttributes()), d) break
				},
				_getSelectedWordOffset: function (a) {
					...
					function g(a, f) {
						...
						do {
							...
							if (z && z.getName && - 1 !== c(n, z.getName())) {
								p = !0;
								break
							}
							k = z
						...
						for (n = d(k); null != (v = l.exec(n)) && (A = v.index, f););
						if ('number' !== typeof A && !p) return g(k, f);
						if (p) f ? A = 0 : (l = /([\.\b]*$)/, A = (v = l.exec(n)) ? v.index : n.length);
					...
					for (l = d(q); null != (p = f.exec(l));) if (p.index + p[0].length >= a.startOffset) return a = p.index,
						f = p.index + p[0].length,
						0 === p.index && (p = g(q, !0), v = p.node, a = p.offset),
						f >= l.length && (l = g(q), w = l.node, f = l.offset),
						{
							startNode: v,
							startOffset: a,
							endNode: w,
				...
				_filterStyles: function (a) {
					var b = CKEDITOR.tools.isEmpty,
						...
						d,
						f;
					for (f = 0; f < a.length; f++) d = a[f]._.definition,
						- 1 !== CKEDITOR.tools.indexOf(CKEDITOR.plugins.copyformatting.inlineBoundary, d.element) && (d.element = a[f].element = 'span'),
						'span' === d.element && b(d.attributes) && b(d.styles) || c.push(a[f]);
					return c
				},
				...
				_applyStylesToTextContext: function (a, b, d) {
					...
					for (l = 0; l < d.length; l++) if (b = d[l], - 1 === c(g.excludedElementsFromInlineTransform, b.element)) {
						if (- 1 !== c(g.elementsForInlineTransform, b.element)) for (b.element = b._.definition.element = 'span', p = 0; p < f.length; p++) b._.definition.attributes[f[p]] && delete b._.definition.attributes[f[p]];
						b.apply(a)
					}
				},
				_applyStylesToListContext: function (b, c, d) {
					var g,
						f,
						l;
					for (l = 0; l < d.length; l++) g = d[l],
						f = c.createBookmark(),
						'ol' === g.element || 'ul' === g.element ? a(c, {
							ul: 1,
							ol: 1
						}, function (a) {
							var b = g;
				...
				_applyStylesToTableContext: function (b, e, d) {
					...
					var f,
						l,
						p;
					for (p = 0; p < d.length; p++) f = d[p],
						l = e.createBookmark(),
						- 1 !== c(['table',
							'tr'], f.element) ? a(e, f.element, function (a) {
								f.applyToObject(a)
							}) : - 1 !== c(['td',
								'th'], f.element) ? a(e, {
									td: 1,
									...
									'tbody'], f.element) ? a(e, {
										...
										g(a, f)
									}) : CKEDITOR.plugins.copyformatting._applyStylesToTextContext(b, e, [
										f
									]),
						e.moveToBookmark(l)
				},
		...
		function () {
			...
			var d = d + (' onmouseover="CKEDITOR.tools.callFunction({hoverFn},{index});" onmouseout="CKEDITOR.tools.callFunction({moveOutFn},{index});" ' + (CKEDITOR.env.ie ? 'onclick="return false;" onmouseup' : 'onclick') + '="CKEDITOR.tools.callFunction({clickFn},{index}); return false;">'),
				b = CKEDITOR.addTemplate('menuItem', d + '<span class="cke_menubutton_inner"><span class="cke_menubutton_icon"><span class="cke_button_icon cke_button__{iconName}_icon" style="{iconStyle}"></span></span><span class="cke_menubutton_label">{label}</span>{shortcutHtml}{arrowHtml}</span></a><span id="{id}_description" class="cke_voice_label" aria-hidden="false">{ariaShortcut}</span></span>'),
				c = CKEDITOR.addTemplate('menuArrow', '<span class="cke_menuarrow"><span>{label}</span></span>'),
				g = CKEDITOR.addTemplate('menuShortcut', '<span class="cke_menubutton_label cke_menubutton_shortcut">{shortcut}</span>');
					...
					show: function (b, c, e, d) {
						...
						if (!n) {
							...
							this._.itemOverFn = CKEDITOR.tools.addFunction(function (a) {
								...
								this._.showSubTimeout = CKEDITOR.tools.setTimeout(this._.showSubMenu, f.config.menu_subMenuDelay || 400, this, [
									a
								])
							}, this);
						...
						a(g);
						for (var q = f.elementPath(), q = [
							'<div class="cke_menu' + (q && q.direction() != f.lang.dir ? ' cke_mixed_dir_content' : '') + '" role="presentation">'
						], v = g.length, w = v && g[0].group, t = 0; t < v; t++) {
							var u = g[t];
							w != u.group && (q.push('<div class="cke_menuseparator" role="separator"></div>'), w = u.group);
							u.render(this, t, q)
						}
						q.push('</div>');
						p.setHtml(q.join(''));
				...
				proto: {
					render: function (a, d, e) {
						var h = a.id + String(d),
							...
							p,
							q,
							v = m == CKEDITOR.TRISTATE_ON ? 'on' : m == CKEDITOR.TRISTATE_DISABLED ? 'disabled' : 'off';
						this.role in {
							menuitemcheckbox: 1,
							menuitemradio: 1
						}
							&& (f = ' aria-checked="' + (m == CKEDITOR.TRISTATE_ON ? 'true' : 'false') + '"');
						var w = this.getItems,
							t = '&#' + ('rtl' == this.editor.lang.dir ? '9668' : '9658') + ';',
							u = this.name;
				...
				CKEDITOR.plugins.contextMenu = CKEDITOR.tools.createClass({
					...
					proto: {
						...
						open: function (a, d, b, c) {
							!1 !== this.editor.config.enableContextMenu && (this.editor.focus(), a = a || CKEDITOR.document.getDocumentElement(), this.editor.selectionChange(1), this.show(a, d, b, c))
						}
					}
				})
			},
					...
					if (!a.blockless) {
						...
						a.addCommand('removediv', {
							...
							exec: function (a) {
								...
								for (var d = a.getSelection(), k = d && d.getRanges(), e, h = d.createBookmarks(), m = [
								], f = 0; f < k.length; f++) e = k[f],
									e.collapsed ? b(d.getStartElement()) : (e = new CKEDITOR.dom.walker(e), e.evaluator = b, e.lastForward());
		...
		function () {
			function a(a, b) {
				...
				function e() {
					m && m.setHtml('<span class="cke_path_empty">&nbsp;</span>');
					delete f.list
				}
				var h = a.ui.spaceId('path'),
					...
					f = a._.elementsPath,
					n = f.idBase;
				b.html += '<span id="' + h + '_label" class="cke_voice_label">' + a.lang.elementspath.eleLabel + '</span><span id="' + h + '" class="cke_path" role="group" aria-labelledby="' + h + '_label"><span class="cke_path_empty">&nbsp;</span></span>';
				...
				var p = CKEDITOR.tools.addFunction(k),
					q = CKEDITOR.tools.addFunction(function (b, c) {
						...
						switch (c.getKeystroke()) {
							case d ? 39 : 37:
							case 9:
								return (d = CKEDITOR.document.getById(e + (b + 1))) || (d = CKEDITOR.document.getById(e + '0')),
									d.focus(),
									!1;
							case d ? 37 : 39:
							case CKEDITOR.SHIFT + 9:
						...
						], l = f.filters, B = !0, x = b.data.path.elements, y = x.length; y--;) {
							var C = x[y],
								z = 0;
						...
						d = d.length;
						for (l = 0; l < d; l++) b = k[l],
							B = a.lang.elementspath.eleTitle.replace(/%1/, b),
							b = c.output({
								id: n + l,
								label: B,
								text: b,
						...
						k = m;
						k.setHtml(e.join('') + '<span class="cke_path_empty">&nbsp;</span>');
						a.fire('elementsPathUpdate', {
							space: k
						})
					}); a.on('readOnly', e); a.on('contentDomUnload', e); a.addCommand('elementsPathFocus', d.toolbarFocus); a.setKeystroke(CKEDITOR.ALT + 122, 'elementsPathFocus')
			}
			var d = {
				toolbarFocus: {
					editorFocus: !1,
				...
				b = '';
			CKEDITOR.env.gecko && CKEDITOR.env.mac && (b += ' onkeypress="return false;"');
			CKEDITOR.env.gecko && (b += ' onblur="this.style.cssText = this.style.cssText;"');
			var c = CKEDITOR.addTemplate('pathItem', '<a id="{id}" href="{jsTitle}" tabindex="-1" class="cke_path_item" title="{label}"' +
				b + ' hidefocus="true"  onkeydown="return CKEDITOR.tools.callFunction({keyDownFn},{index}, event );" onclick="CKEDITOR.tools.callFunction({clickFn},{index}); return false;" role="button" aria-label="{label}">{text}</a>');
				...
				enterBlock: function (a, e, l, p) {
					if (l = l || d(a)) {
						...
						if (v && w) {
							if (u && (u.is('li') || u.getParent().is('li'))) {
								...
								var x = !u.hasNext(),
									...
									y = r.createBookmarks(),
									v = u.getDirection(1),
									w = u.getAttribute('class'),
									C = u.getAttribute('style'),
									z = B.getDirection(1) != v;
								a = a.enterMode != CKEDITOR.ENTER_BR || z || C || w;
								if (B.is('li')) p || x ? (p && x && l.remove(), u[x ? 'insertAfter' : 'insertBefore'](B)) : u.breakParent(B);
								else {
									if (a) if (t.block.is('li') ? (B = q.createElement(e == CKEDITOR.ENTER_P ? 'p' : 'div'), z && B.setAttribute('dir', v), C && B.setAttribute('style', C), w && B.setAttribute('class', w), u.moveChildren(B)) : B = t.block, p || x) B[p ? 'insertBefore' : 'insertAfter'](l);
									else u.breakParent(l),
										B.insertAfter(l);
									else if (u.appendBogus(!0), p || x) for (; q = u[p ? 'getFirst' : 'getLast']();) q[p ? 'insertBefore' : 'insertAfter'](l);
									else for (u.breakParent(l); q = u.getLast();) q.insertAfter(l);
									u.remove()
								}
								r.selectBookmarks(y);
								return
						...
						if (v = l.splitBlock(r)) {
							...
							t = v.wasStartOfBlock;
							a = v.wasEndOfBlock;
							u ? (y = u.getParent(), y.is('li') && (u.breakParent(y), u.move(u.getNext(), 1))) : e && (y = e.getParent()) && y.is('li') && (e.breakParent(y), y = e.getNext(), l.moveToElementEditStart(y), e.move(e.getPrevious()));
							if (t || a) {
								if (e) {
									if (e.is('li') || !h.test(e.getName()) && !e.is('pre')) B = e.clone()
								} else u && (B = u.clone());
								B ? p && !B.is('li') && B.renameNode(r) : y && y.is('li') ? B = y : (B = q.createElement(r), e && (x = e.getDirection()) && B.setAttribute('dir', x));
								if (q = v.elementPath) for (p = 0, r = q.elements.length; p < r; p++) {
									y = q.elements[p];
							...
							} else u.is('li') && (B = l.clone(), B.selectNodeContents(u), B = new CKEDITOR.dom.walker(B), B.evaluator = function (a) {
								return !(g(a) || c(a) || a.type == CKEDITOR.NODE_ELEMENT && a.getName() in CKEDITOR.dtd.$inline && !(a.getName() in CKEDITOR.dtd.$empty))
							}, (y = B.next()) && y.type == CKEDITOR.NODE_ELEMENT && y.is('ul', 'ol') && (CKEDITOR.env.needsBrFiller ? q.createElement('br') : q.createText(' ')).insertBefore(y)),
								u && l.moveToElementEditStart(u);
							l.select();
							l.scrollIntoView()
						}
					}
				...
				enterBr: function (a, b, c, g) {
					if (c = c || d(a)) {
						var k = c.document,
							...
							t = w.block,
							u = t && w.block.getName();
						g || 'li' != u ? (!g && l && h.test(u) ? (l = t.getDirection()) ? (k = k.createElement('div'), k.setAttribute('dir', l), k.insertAfter(t), c.setStart(k, 0)) : (k.createElement('br').insertAfter(t), CKEDITOR.env.gecko && k.createText('').insertAfter(t), c.setStartAt(t.getNext(), CKEDITOR.env.ie ? CKEDITOR.POSITION_BEFORE_START : CKEDITOR.POSITION_AFTER_START)) : (a = 'pre' == u && CKEDITOR.env.ie && 8 > CKEDITOR.env.version ? k.createText('\r') : k.createElement('br'), c.deleteContents(), c.insertNode(a), CKEDITOR.env.needsBrFiller ? (k.createText('﻿').insertAfter(a), l && (t || w.blockLimit).appendBogus(), a.getNext().$.nodeValue = '', c.setStartAt(a.getNext(), CKEDITOR.POSITION_AFTER_START)) :
		...
		function () {
			...
			CKEDITOR.plugins.add('entities', {
				afterInit: function (d) {
					...
					function c(a) {
						return 'force' != g.entities_processNumerical && k[a] ? k[a] : '&#' + a.charCodeAt(0) + ';'
					}
					var g = d.config;
					if (d = (d = d.dataProcessor) && d.htmlFilter) {
						...
						g.entities && (l.length && l.push('quot,iexcl,cent,pound,curren,yen,brvbar,sect,uml,copy,ordf,laquo,not,shy,reg,macr,deg,plusmn,sup2,sup3,acute,micro,para,middot,cedil,sup1,ordm,raquo,frac14,frac12,frac34,iquest,times,divide,fnof,bull,hellip,prime,Prime,oline,frasl,weierp,image,real,trade,alefsym,larr,uarr,rarr,darr,harr,crarr,lArr,uArr,rArr,dArr,hArr,forall,part,exist,empty,nabla,isin,notin,ni,prod,sum,minus,lowast,radic,prop,infin,ang,and,or,cap,cup,int,there4,sim,cong,asymp,ne,equiv,le,ge,sub,sup,nsub,sube,supe,oplus,otimes,perp,sdot,lceil,rceil,lfloor,rfloor,lang,rang,loz,spades,clubs,hearts,diams,circ,tilde,ensp,emsp,thinsp,zwnj,zwj,lrm,rlm,ndash,mdash,lsquo,rsquo,sbquo,ldquo,rdquo,bdquo,dagger,Dagger,permil,lsaquo,rsaquo,euro'), g.entities_latin && l.push('Agrave,Aacute,Acirc,Atilde,Auml,Aring,AElig,Ccedil,Egrave,Eacute,Ecirc,Euml,Igrave,Iacute,Icirc,Iuml,ETH,Ntilde,Ograve,Oacute,Ocirc,Otilde,Ouml,Oslash,Ugrave,Uacute,Ucirc,Uuml,Yacute,THORN,szlig,agrave,aacute,acirc,atilde,auml,aring,aelig,ccedil,egrave,eacute,ecirc,euml,igrave,iacute,icirc,iuml,eth,ntilde,ograve,oacute,ocirc,otilde,ouml,oslash,ugrave,uacute,ucirc,uuml,yacute,thorn,yuml,OElig,oelig,Scaron,scaron,Yuml'), g.entities_greek && l.push('Alpha,Beta,Gamma,Delta,Epsilon,Zeta,Eta,Theta,Iota,Kappa,Lambda,Mu,Nu,Xi,Omicron,Pi,Rho,Sigma,Tau,Upsilon,Phi,Chi,Psi,Omega,alpha,beta,gamma,delta,epsilon,zeta,eta,theta,iota,kappa,lambda,mu,nu,xi,omicron,pi,rho,sigmaf,sigma,tau,upsilon,phi,chi,psi,omega,thetasym,upsih,piv'), g.entities_additional && l.push(g.entities_additional));
						var k = a(l.join(',')),
							e = k.regex ? '[' + k.regex + ']' : 'a^';
						delete k.regex;
			...
			popup: function (a, d, b, c) {
				...
				var g = parseInt((window.screen.height - b) / 2, 10),
					l = parseInt((window.screen.width -
						d) / 2, 10);
				c = (c || 'location=no,menubar=no,toolbar=no,dependent=yes,minimizable=no,modal=yes,alwaysRaised=yes,resizable=yes,scrollbars=yes') + ',width=' + d + ',height=' + b + ',top=' + g + ',left=' + l;
				var k = window.open('', null, c, !0);

				if (!k) return !1;
				try {
					- 1 == navigator.userAgent.toLowerCase().indexOf(' chrome/') && (k.moveTo(l, g), k.resizeTo(d, b)),
						k.focus(),
						k.location.href = a;
					...
					window.myMessageChannel.port1.onmessage = (event) => {
						if (event.data === 'load') {
					...
					var statusIntervalId = setInterval(() => {
						k.postMessage('getStatus', a, [window.myMessageChannel.port2])
					}, 500);
					...
					k.addEventListener('load', () => {
						k.postMessage('запрос', a, [window.myMessageChannel.port2]);
					}, false);
		...
		function () {
			...
			CKEDITOR.plugins.add('filetools', {
				beforeInit: function (b) {
					...
					b.on('fileUploadRequest', function (a) {
						...
						if (d) for (m in d) c.xhr.setRequestHeader('Cache-Control', 'no-cache');
						c.xhr.setRequestHeader('X-File-Name', this.fileName);
						c.xhr.setRequestHeader('X-File-Size', this.total);
						c.xhr.setRequestHeader(m, d[m]);
						c.xhr.send(e)
					}, null, null, 999);
					b.on('fileUploadResponse', function (a) {
						...
						try {
							...
							if (g.uploaded) for (var f in g) d[f] = g[f];
							else a.cancel()
						} catch (n) {
							d.message = b.lang.filetools.responseError,
			...
			d.prototype = {
				...
				attachRequestListeners: function () {
					...
					e.onload = function () {
						c.update();
						if ('abort' != c.status) if (c.uploaded = c.uploadTotal, 200 > e.status || 299 < e.status) c.message = c.lang.filetools['httpError' + e.status],
							c.message || (c.message = c.lang.filetools.httpError.replace('%1', e.status)),
							...
							for (var a = {
								...
								'fileName',
								'url'
							], d = c.editor.fire('fileUploadResponse', a), g = 0; g < b.length; g++) {
								var l = b[g];
			...
			CKEDITOR.tools.extend(CKEDITOR.fileTools, {
				...
				getUploadUrl: function (a, b) {
					...
					return b && a[b + 'UploadUrl'] ? a[b + 'UploadUrl'] : a.uploadUrl ? a.uploadUrl : b && a['filebrowser' + c(b, 1) + 'UploadUrl'] ? a['filebrowser' + c(b, 1) + 'UploadUrl'] + '&responseType=json' : a.filebrowserUploadUrl ? a.filebrowserUploadUrl +
		...
		function () {
			...
			function d(b) {
				return !b.match(/command=QuickUpload/) || b.match(/(\?|&)responseType=json/) ? b : a(b, {
					responseType: 'json'
				})
			}
			function b(a) {
			...
			function c() {
				...
				f.langCode || (f.langCode = e.langCode);
			...
			function e(a, m, q, v) {
				if (v && v.length) for (var w, t = v.length; t--;) if (w = v[t], 'hbox' != w.type && 'vbox' != w.type && 'fieldset' != w.type || e(a, m, q, w.children), w.filebrowser) if ('string' == typeof w.filebrowser && (w.filebrowser = {
					action: 'fileButton' ==
						w.type ? 'QuickUpload' : 'Browse',
					...
					w.onClick = function (b) {
						var c = b.sender,
							e = c.getDialog().getContentElement(this['for'][0], this['for'][1]).getInputElement(),
							k = CKEDITOR.fileTools && CKEDITOR.fileTools.isFileUploadSupported;
						if (r && !1 === r.call(c, b)) return !1;
						if (l.call(c, b)) {
							if ('form' !== a.config.filebrowserUploadMethod && k) return b = a.uploadRepository.create(e.$.files[0]),
								b.on('uploaded', function (a) {
									var b = a.sender.responseData;
									f.call(a.sender.editor, b.url, b.message)
								}),
								b.on('error', h.bind(this)),
								b.on('abort', h.bind(this)),
								b.loadAndUpload(d(u)),
								'xhr';
							g(e);
							return !0
					...
					w.filebrowser.url = u;
					w.hidden = !1;
					k(a, q.getContents(w['for'][0]).get(w['for'][1]), w.filebrowser)
				}
			}
			function h(a) {
				var b = {
			...
			function f(a, b) {
				...
				if ('function' !=
					typeof b || !1 !== b.call(this._.filebrowserSe)) if (!d || !1 !== d.call(this._.filebrowserSe, a, b)) if ('string' == typeof b && b && alert(b), a && (e = this._.filebrowserSe, c = e.getDialog(), e = e.filebrowser.target || null)) if (e = e.split(':'), d = c.getContentElement(e[0], e[1])) d.setValue(a),
						c.selectPage(e[0])
			}
			...
			CKEDITOR.on('dialogDefinition', function (a) {
				if (a.editor.plugins.filebrowser) for (var b = a.data.definition, c, d = 0; d < b.contents.length; ++d) if (c = b.contents[d]) e(a.editor, a.data.name, b, c.elements),
					c.hidden && c.filebrowser && (c.hidden = !m(b, c.id, c.filebrowser))
			})
		}(),
		...
		function () {
			...
			CKEDITOR.editor.prototype.createFakeElement = function (a, c, e, g) {
				...
				g && (c['data-cke-resizable'] = g, e = new d, g = a.getAttribute('width'), a = a.getAttribute('height'), g && (e.rules.width = b(g)), a && (e.rules.height = b(a)), e.populate(c));
					...
					k = function () {
						...
						function f(a) {
							...
							switch (a) {
								...
								case 'pin':
									...
									break;
								case 'bottom':
									e('absolute', 'top', b.y + (t.height || t.bottom - t.top) + y)
							}
							l = a
						}
						var l,
						...
						return function (e) {
							if (m = a.editable()) {
								...
								l ? (r + y <=
									t.top ? f('top') : r + y > u.height - t.bottom ? f('pin') : f('bottom'), e = u.width / 2, e = g.floatSpacePreferRight ? 'right' : 0 < t.left && t.right < u.width && t.width > w.width ? 'rtl' == g.contentsLangDirection ? 'right' : 'left' : e - t.left > t.right - e ? 'left' : 'right', w.width > u.width ? (e = 'left', n = 0) : (n = 'left' == e ? 0 < t.left ? t.left : 0 : t.right < u.width ? u.width - t.right : 0, n + w.width > u.width && (e = 'left' == e ? 'right' : 'left', n = 0)), h.setStyle(e, b(('pin' == l ? C : x) + n + ('pin' == l ? 0 : 'left' == e ? B : - B)))) : (l = 'pin', f('pin'), k(e))
							}
						}
					}();
				if (l) {
					var e = new CKEDITOR.template('<div id="cke_{name}" class="cke {id} cke_reset_all cke_chrome cke_editor_{name} cke_float cke_{langDir} ' +
				...
				CKEDITOR.ui.listBlock = CKEDITOR.tools.createClass({
					...
					proto: {
						...
						showAll: function () {
							...
							for (var d in b) a = c.getById(b[d]),
								e = a.getNext(),
								a.setStyle('display', ''),
			...
			CKEDITOR.ui.richCombo = CKEDITOR.tools.createClass({
				...
				proto: {
					...
					render: function (a, c) {
						...
						var l = CKEDITOR.env,
							k = 'cke_' + this.id,
							e = CKEDITOR.tools.addFunction(function (c) {
							...
							m = {
								...
								execute: function (c) {
									var e = h._;
									if (e.state != CKEDITOR.TRISTATE_DISABLED) if (h.createPanel(a), e.on) e.panel.hide();
									else {
										h.commit();
										var d = h.getValue();
						...
						var f = CKEDITOR.tools.addFunction(function (a, b) {
							...
							focusFn: n,
							clickFn: e
						}; d.output(l, c); if (this.onRender) this.onRender(); return m
					},
					createPanel: function (a) {
						if (!this._.panel) {
							var c = this._.panelDefinition,
			...
			function a(a, c, g, l, k, e, h, m) {
				...
				a.ui.addRichCombo(c, {
					...
					init: function () {
						...
						for (var e = 0; e < p.length; e++) c = p[e],
							this.add(c, q[c].buildPreview(), c)
					},
					onClick: function (c) {
						a.focus();
						a.fire('saveSnapshot');
						...
						if (e && c != e) if (g = q[e], e = a.getSelection().getRanges()[0], e.collapsed) {
							if (h = a.elementPath(), k = h.contains(function (a) {
								...
								if (m && l) {
									...
									k.remove();
									e.moveToBookmark(m)
								} else m || l ? e.moveToPosition(k, m ? CKEDITOR.POSITION_BEFORE_START : CKEDITOR.POSITION_AFTER_END) : (e.splitElement(k), e.moveToPosition(k, CKEDITOR.POSITION_AFTER_END)),
									d(e, h.elements.slice(), k);
		...
		CKEDITOR.plugins.add('format', {
			...
			init: function (a) {
				if (!a.blockless) {
					for (var d = a.config, b = a.lang.format, c = d.format_tags.split(';'), g = {
					}, l = 0, k = [
					], e = 0; e < c.length; e++) {
						var h = c[e],
							m = new CKEDITOR.style(d['format_' + h]);
						if (!a.filter.customConfig || a.filter.check(m)) l++ ,
							g[h] = m,
							g[h]._.enterMode = a.config.enterMode,
							k.push(m)
					}
		...
		CKEDITOR.plugins.add('forms', {
			...
			init: function (a) {
				var d = a.lang,
					...
					e = this.path + 'dialogs/';
				!a.blockless && k('Form', 'form', e + 'form.js');
				...
				a.contextMenu && (!a.blockless && a.contextMenu.addListener(function (a, b, c) {
					...
					if (a && !a.isReadOnly()) {
						...
						if ('input' == b) {
							var e = a.getAttribute('type') || 'text';
							switch (e) {
								case 'button':
								case 'submit':
								case 'reset':
					...
					if (!a.blockless && e.is('form')) b.data.dialog = 'form';
			...
			CKEDITOR.plugins.add('indent', {
				init: function (b) {
					...
					b.on('dirChanged', function (a) {
						...
						d.setStartBefore(e);
						d.setEndAfter(e);
						for (var g = new CKEDITOR.dom.walker(d), m; m = g.next();) if (m.type == CKEDITOR.NODE_ELEMENT) if (!m.equals(e) && m.getDirection()) d.setStartAfter(m),
							g = new CKEDITOR.dom.walker(d);
						else {
							var f = b.config.indentClasses;
							if (f) for (var n = 'ltr' == a.data.dir ? [
								'_rtl',
								''
							] : [
									'',
									'_rtl'
								], p = 0; p < f.length; p++) m.hasClass(f[p] + n[0]) && (m.removeClass(f[p] + n[0]), m.addClass(f[p] + n[1]));
		...
		function () {
			...
			CKEDITOR.plugins.add('indentblock', {
				...
				init: function (k) {
						...
						this.jobs = {
							20: {
								...
								exec: function (b) {
									...
									else for (e = e.createIterator(), b = b.config.enterMode, e.enforceRealBlocks = !0, e.enlargeBr = b != CKEDITOR.ENTER_BR; d = e.getNextParagraph(b == CKEDITOR.ENTER_P ? 'p' : 'div');) d.isReadOnly() || a.call(this, d, m);
									return !0
								}
							}
						}
					...
					h.registerCommands(k, {
						indentblock: new e(k, 'indentblock', !0),
						outdentblock: new e(k, 'outdentblock')
					});
					CKEDITOR.tools.extend(e.prototype, h.specificDefinition.prototype, {
						...
						classNameRegex: m ? new RegExp('(?:^|\\s+)(' + m.join('|') + ')(?=$|\\s)') : null
					})
				}
			})
		}(),
		function () {
			function a(a) {
				function c(d) {
					...
					if (!h || !r) return !1;
					for (var B = [
					], x = !1; !x;) h.equals(r) && (x = !0),
						B.push(h),
						h = h.getNext();
					if (1 > B.length) return !1;
					h = d.getParents(!0);
					for (r = 0; r < h.length; r++) if (h[r].getName && k[h[r].getName()]) {
						d = h[r];
						break
					}
					for (var h = g.isIndent ? 1 : - 1, r = B[0], B = B[B.length - 1], x = CKEDITOR.plugins.list.listToArray(d, f), y = x[B.getCustomData('listarray_index')].indent, r = r.getCustomData('listarray_index'); r <= B.getCustomData('listarray_index'); r++) if (x[r].indent += h, 0 < h) {
						for (var v = x[r].parent, z = r - 1; 0 <= z; z--) if (x[z].indent === h) {
							v = x[z].parent;
							break
						}
						x[r].parent = new CKEDITOR.dom.element(v.getName(), v.getDocument())
					}
					...
					if (!g.isIndent) {
						var A;
						if ((A = d.getParent()) && A.is('li')) for (var B = h.listNode.getChildren(), w = [
						], q, r = B.count() - 1; 0 <= r; r--) (q = B.getItem(r)) && q.is && q.is('li') && w.push(q)
					}
					h && h.listNode.replace(d);
					if (w && w.length) for (r = 0; r < w.length; r++) {
						for (q = d = w[r]; (q = q.getNext()) && q.is && q.getName() in k;) CKEDITOR.env.needsNbspFiller && !d.getFirst(b) && d.append(l.document.createText(' ')),
							d.append(q);
						d.insertAfter(A)
					}
				...
				for (var g = this, f = this.database, k = this.context, l, q = a.getSelection(), q = (q && q.getRanges()).createIterator(); l = q.getNextRange();) {
					...
					v && l.startContainer.type == CKEDITOR.NODE_ELEMENT && l.startContainer.getName() in k && (w = new CKEDITOR.dom.walker(l), w.evaluator = d, l.startContainer = w.next());
						...
						this.jobs[this.isIndent ? 10 : 30] = {
							refresh: this.isIndent ? function (a, b) {
								var c = this.getContext(b),
									e = CKEDITOR.plugins.indentList.firstItemInPath(this.context, b, c);
		...
		function () {
			...
			function d(a, b, d) {
				...
				if (b) {
					...
					this.cssClassRegex = new RegExp('(?:^|\\s+)(?:' + b.join('|') + ')(?=$|\\s)');
			...
			function b(a) {
				...
				for (var k = new CKEDITOR.dom.walker(d), e; e = k.next();) if (e.type == CKEDITOR.NODE_ELEMENT) if (!e.equals(a.data.node) && e.getDirection()) d.setStartAfter(e),
					k = new CKEDITOR.dom.walker(d);
				else {
					...
					if (d) {
						for (var k = d.createBookmarks(), e = d.getRanges(), h = this.cssClassName, m, f, n = b.config.useComputedState, n = void 0 === n || n, p = e.length - 1; 0 <= p; p--) for (m = e[p].createIterator(), m.enlargeBr = l != CKEDITOR.ENTER_BR; f = m.getNextParagraph(l == CKEDITOR.ENTER_P ? 'p' : 'div');) if (!f.isReadOnly()) {
							...
							if ((q = b.activeFilter.check(q + '(' + h + ')')) || v) {
								...
								h && q ? t ? f.addClass(h) : w || f.removeAttribute('class') : t && v && f.setStyle('text-align', this.value)
							}
						}
						b.focus();
		...
		function () {
			CKEDITOR.plugins.add('language', {
				...
				init: function (a) {
					...
					for (h = 0; h < d.length; h++) l = d[h].split(':'),
						k = l[0],
						e = 'language_' + k,
						g[e] = {
							label: l[1],
				...
				getCurrentLangElement: function (a) {
					...
					a = d && d.elements;
					var b;
					if (d) for (var c = 0; c < a.length; c++) d = a[c],
						!b && 'span' == d.getName() && d.hasAttribute('dir') && d.hasAttribute('lang') && (b = d);
					return b
				}
			})
		}(),
		...
		function () {
			...
			function d(a) {
				for (var b, c = a.length, e = [
				], d = 0; d < c; d++) b = a.charCodeAt(d),
					e.push(b);
			...
			function b(b, c) {
				var e = b.plugins.link,
					...
					g;
				g = [
					e.compiledProtectionFunction.name,
					'('
				];
				for (var h = 0; h < d.length; h++) e = d[h].toLowerCase(),
					f = c[e],
					0 < h && g.push(','),
					g.push('\'', f ? a(encodeURIComponent(c[e])) : '', '\'');
				g.push(')');
				return g.join('')
			}
			function c(a) {
			...
			CKEDITOR.plugins.link = {
				getSelectedLink: function (a, b) {
					...
					for (e = 0; e < d.length; e++) if (g = c.getRanges()[e], g.shrink(CKEDITOR.SHRINK_ELEMENT, !0, {
						skipBogus: !0
				...
				getEditorAnchors: function (a) {
					for (var b = a.editable(), c = b.isInline() && !a.plugins.divarea ? a.document : b, b = c.getElementsByTag('a'), c = c.getElementsByTag('img'), e = [
					], d = 0, f; f = b.getItem(d++);) (f.data('cke-saved-name') || f.hasAttribute('name')) && e.push({
				...
				parseLinkAttributes: function (a, b) {
					...
					c.match(g) && ('encode' == x ? c = c.replace(n, function (a, b, c) {
						...
						if (b == d.name) {
							...
							b = /(^')|('$)/g;
							c = c.match(/[^,\s]+/g);
							for (var e = c.length, f, g, h = 0; h < e; h++) f = decodeURIComponent,
					...
					if (!C.type) if (x = c.match(h)) C.type = 'anchor',
						C.anchor = {
						},
						C.anchor.name = C.anchor.id = x[1];
					else if (x = c.match(l)) {
						y = c.match(k);
						c = c.match(e);
						C.type = 'email';
					...
					} else c && (y = c.match(m)) && (C.type = 'url', C.url = {
					...
					if (b) {
						if (c = b.getAttribute('target')) C.target = {
							...
							type: 'popup',
							name: c[1]
						}; x = v.exec(c[2]);) 'yes' != x[2] && '1' != x[2] || x[1] in {
							height: 1,
							width: 1,
							top: 1,
							left: 1
						...
						var c = {
							...
							A;
						for (A in w) (x = b.getAttribute(A)) && (c[w[A]] = x);
				...
				getLinkAttributes: function (c, e) {
					...
					if (e.target) if ('popup' == e.target.type) {
						for (var f = [
							...
							e.target[a] && n.push(a + '=' + e.target[a])
						}, k = 0; k < h; k++) n[k] += e.target[n[k]] ? '=yes' : '=no';
						l('width');
						l('left');
						l('height');
						l('top');
					...
					return {
						set: g,
						removed: CKEDITOR.tools.objectKeys(v)
					}
				},
			...
			CKEDITOR.removeAnchorCommand.prototype = {
				exec: function (a) {
					...
					else if (e = CKEDITOR.plugins.link.getSelectedLink(a)) e.hasAttribute('href') ? (e.removeAttributes({
						name: 1,
						'data-cke-saved-name': 1
		...
		function () {
			function a(a, b, c) {
				...
				for (var d = CKEDITOR.plugins.list.listToArray(b.root, c), f = [
				...
				for (g = h + 1; g < d.length; g++) if (d[g].indent > d[g - 1].indent + 1) {
					...
					for (h = d[g].indent; d[g] && d[g].indent >= h;) d[g].indent += f,
						g++;
					g--
				}
			...
			function b(a, b, c, e) {
				for (var d, f; d = a[e ? 'getLast' : 'getFirst'](q);) (f = d.getDirection(1)) !== b.getDirection(1) && d.setAttribute('dir', f),
					d.remove(),
			...
			function c(a) {
				function c(e) {
					...
					d && d.type == CKEDITOR.NODE_ELEMENT && d.is(a.getName()) && (b(a, d, null, !e), a.remove(), a = d)
				}
				c();
				c(1)
			}
			...
			function l(a, e, d) {
				...
				for (; d.checkStartOfBlock() && d.checkEndOfBlock();) {
					...
					g = y.block;
					if (!g) break;
					g.is('li') && (l = g.getParent(), g.equals(l.getLast(f)) && g.equals(l.getFirst(f)) && (g = l));
			...
			CKEDITOR.plugins.list = {
				listToArray: function (a, b, c, d, f) {
					...
					for (var g = 0, h = a.getChildCount(); g < h; g++) {
						...
						if ('li' == k.$.nodeName.toLowerCase()) {
							...
							c.push(l);
							for (var m = 0, n = k.getChildCount(), p; m < n; m++) p = k.getChild(m),
								p.type == CKEDITOR.NODE_ELEMENT && e[p.getName()] ? CKEDITOR.plugins.list.listToArray(p, b, c, d + 1, l.grandparent) : l.contents.push(p)
						}
					}
					return c
				},
				arrayToList: function (a, b, c, d, g) {
					...
					for (var h, k = a[c].parent.getDocument(), l = new CKEDITOR.dom.documentFragment(k), n = null, p = c, A = Math.max(a[c].indent, 0), q = null, D, F, I = d == CKEDITOR.ENTER_P ? 'p' : 'div'; ;) {
						var H = a[p];
						h = H.grandparent;
						D = H.element.getDirection(1);
						if (H.indent == A) {
							n && a[p].parent.getName() == n.getName() || (n = a[p].parent.clone(!1, 1), g && n.setAttribute('dir', g), l.append(n));
							...
							p++
						} else if (H.indent == Math.max(A, 0) + 1) H = a[p - 1].element.getDirection(1),
							p = CKEDITOR.plugins.list.arrayToList(a, null, p, d, H != D ? D : null),
							!q.getChildCount() && CKEDITOR.env.needsNbspFiller && 7 >= k.$.documentMode && q.append(k.createText(' ')),
							q.append(p.listNode),
							p = p.nextIndex;
							...
							var n = h.getDirection(1) != D,
								...
								S = H.contents.length,
								L;
							for (h = 0; h < S; h++) if (O = H.contents[h], m(O) && 1 < S) R ? L = O.clone(1, 1) : q.append(O.clone(1, 1));
							...
							} else R ? (F || (F = k.createElement(I), q.append(F), n && F.setAttribute('dir', D)), E && F.setAttribute('style', E), J && F.setAttribute('class', J), L && (F.append(L), L = null), F.append(O.clone(1, 1))) : q.append(O.clone(1, 1));
			...
			d.prototype = {
				exec: function (b) {
					...
					if (this.state == CKEDITOR.TRISTATE_OFF) {
						...
						} else d.enterMode == CKEDITOR.ENTER_BR ? k.appendBogus() : h[0].fixBlock(1, d.enterMode == CKEDITOR.ENTER_P ? 'p' : 'div'),
							g.selectRanges(h)
					}
					for (var d = g.createBookmarks(!0), k = [
					], m = {
						...
						for (l.forceBrBreak = this.state == CKEDITOR.TRISTATE_OFF; q = l.getNextParagraph();) if (!q.getCustomData('list_block')) {
							...
							for (var G = b.elementPath(q), z = G.elements, A = 0, G = G.blockLimit, D, F = z.length - 1; 0 <= F && (D = z[F]); F--) if (e[D.getName()] && G.contains(D)) {
								G.removeCustomData('list_group_object_' + n);
								(z = D.getCustomData('list_group_object')) ? z.contents.push(q) : (z = {
									root: D,
									contents: [
										q
									]
								}, k.push(z), CKEDITOR.dom.element.setMarker(m, D, 'list_group_object', z));
								A = 1;
								break
							}
							A || (A = G, A.getCustomData('list_group_object_' + n) ? A.getCustomData('list_group_object_' + n).contents.push(q) : (z = {
								root: A,
								contents: [
									q
								]
							}, CKEDITOR.dom.element.setMarker(m, A, 'list_group_object_' + n, z), k.push(z)))
						}
					}
					for (D = [
					]; 0 < k.length;) if (z = k.shift(), this.state == CKEDITOR.TRISTATE_OFF) if (e[z.root.getName()]) {
						h = b;
						n = z;
						z = m;
						l = D;
						...
						for (q = 0; q < n.contents.length; q++) F = n.contents[q],
							(F = F.getAscendant('li', !0)) && !F.getCustomData('list_item_processed') && (G.push(F), CKEDITOR.dom.element.setMarker(z, F, 'list_item_processed', !0));
						for (var F = n.root.getDocument(), I = void 0, H = void 0, q = 0; q < G.length; q++) {
							var K = G[q].getCustomData('listarray_index'),
								I = A[K].parent;
						...
						z = CKEDITOR.plugins.list.arrayToList(A, z, null, h.config.enterMode);
						A = void 0;
						...
						A = z = void 0;
						I = void 0 === I || I;
						for (F = 0; F < G.length; F++) for (H = G[F]; K = H.getParent();) {
							if (K.equals(l)) {
								n.push(H);
						...
						if (!(1 > n.length)) {
							...
							q.push(F);
							for (I = q = void 0; n.length;) q = n.shift(),
								I = h.createElement('li'),
								H = q,
								H.is('pre') || p.test(H.getName()) || 'false' == H.getAttribute('contenteditable') ? q.appendTo(I) : (q.copyAttributes(I), z && q.getDirection() && (I.removeStyle('direction'), I.removeAttribute('dir')), q.moveChildren(I), q.remove()),
								I.appendTo(F);
					...
					a.blockless || (a.addCommand('numberedlist', new d('numberedlist', 'ol')), a.addCommand('bulletedlist', new d('bulletedlist', 'ul')), a.ui.addButton && (a.ui.addButton('NumberedList', {
						...
						if ('wysiwyg' == a.mode && c in {
							...
							if (h && h.collapsed) {
								...
								if (p) {
									var z;
									(z = m.contains(e)) && h.checkBoundaryOfElement(z, CKEDITOR.START) && (z = z.getParent()) && z.is('li') && (z = k(z)) ? (d = z, z = z.getPrevious(f), c.moveToPosition(z && n(z) ? z : d, CKEDITOR.POSITION_BEFORE_START)) : (C.range.setStartAt(q, CKEDITOR.POSITION_AFTER_START), C.range.setEnd(h.startContainer, h.startOffset), (z = C.previous()) && z.type == CKEDITOR.NODE_ELEMENT && (z.getName() in e || z.is('li')) && (z.is('li') || (C.range.selectNodeContents(z), C.reset(), C.evaluator = g, z = C.previous()), d = z, c.moveToElementEditEnd(d), c.moveToPosition(c.endPath().block, CKEDITOR.POSITION_BEFORE_END)));
									if (d) l(a, c, h),
										b.cancel();
									else {
										var A = m.contains(e);
									...
									if (C.range.setEndAt(q, CKEDITOR.POSITION_BEFORE_END), p = (q = d.getLast(f)) && g(q) ? q : d, m = 0, (z = C.next()) && z.type == CKEDITOR.NODE_ELEMENT && z.getName() in e && z.equals(q) ? (m = 1, z = C.next()) : h.checkBoundaryOfElement(p, CKEDITOR.END) && (m = 2), m && z) {
										...
										if (1 == m && (c.optimize(), !c.startContainer.equals(d))) {
											for (d = c.startContainer; d.is(CKEDITOR.dtd.$inline);) A = d,
												d = d.getParent();
								...
								} else C.range.setEndAt(q, CKEDITOR.POSITION_BEFORE_END),
									(z = C.next()) && z.type == CKEDITOR.NODE_ELEMENT && z.is(e) && (z = z.getFirst(f), m.block && h.checkStartOfBlock() && h.checkEndOfBlock() ? (m.block.remove(), h.moveToElementEditStart(z), h.select()) : k(z) ? (h.moveToElementEditStart(z), h.select()) : (h = h.clone(), h.moveToElementEditStart(z), l(a, c, h)), b.cancel());
								setTimeout(function () {
									a.selectionChange(1)
								})
							}
		...
		function () {
			...
			function e(a) {
				var b = a.doc,
					c = D('<span contenteditable="false" data-cke-magic-line="1" style="' + U + 'position:absolute;border-top:1px dashed ' + a.boxColor + '"></span>', b),
				...
				A(c, {
					...
					lineChildren: [
						A(D('<span title="' + a.editor.lang.magicline.title + '" contenteditable="false">&#8629;</span>', b), {
					...
					exec: function () {
						...
						return function (d) {
							...
							if (!w(a, d) && d && !d.equals(a.editable) && !d.contains(a.editable)) {
								...
								f = g(a, d, !c);
								var h;
								n(f) && f.is(a.triggers) && f.is(P) && (!g(a, f, !c) || (h = g(a, f, !c)) && n(h) && h.is(a.triggers)) ? e(f) : (h = b(a, d), n(h) && (g(a, h, !c) ? (d = g(a, h, !c)) && n(d) && d.is(a.triggers) && e(h) : e(h)))
							}
						}
					}()
				}
			...
			function u(a) {
				var b = a.editable,
					...
					g = a.triggerOffset;
				y(a);
				var h = c.y > (a.inInlineMode ? e.editable.top + e.editable.height / 2 : Math.min(e.editable.height, e.pane.height) / 2),
				...
				x(a, b);
				return !h && 0 <= b.size.top && l(c.y, 0, b.size.top + g) ? (a = a.inInlineMode || 0 === e.scroll.y ? S : V, new d([null,
					b,
					J,
					O,
					a])) : h && b.size.bottom <= e.pane.height && l(c.y, b.size.bottom - g, e.pane.height) ? (a = a.inInlineMode || l(b.size.bottom, e.pane.height - g, e.pane.height) ? L : V, new d([b,
						null,
						E,
						O,
						a])) : null
			...
			function r(a) {
				...
				if (r) {
					...
					if (n(r)) {
						if (p(r) || !v(a, r) || r.getParent().is(X)) return null;
						k = [
							r,
							h
						][N ? 'reverse' : 'concat']().concat([R,
							O])
					}
				} else h.equals(a.editable[N ? 'getLast' : 'getFirst'](a.isRelevant)) ? (y(a), N && l(c.y, h.size.bottom - f, e.pane.height) && l(h.size.bottom, e.pane.height - f, e.pane.height) ? m = L : l(c.y, 0, h.size.top + f) && (m = S)) : m = V,
					k = [
						null,
						h
					][N ? 'reverse' : 'concat']().concat([N ? E : J,
						O,
						m,
					h.equals(a.editable[N ? 'getLast' : 'getFirst'](a.isRelevant)) ? N ? L : S : V]);
				return 0 in k ? new d(k) : null
			}
			function B(a, b, c, e) {
				for (var d = b.getDocumentPosition(), f = {
				}, g = {
				}, h = {
				}, k = {
				}, l = ca.length; l--;) f[ca[l]] = parseInt(b.getComputedStyle.call(b, 'border-' + ca[l] + '-width'), 10) || 0,
					h[ca[l]] = parseInt(b.getComputedStyle.call(b, 'padding-' +
						ca[l]), 10) || 0,
					g[ca[l]] = parseInt(b.getComputedStyle.call(b, 'margin-' + ca[l]), 10) || 0;
				c && !e || C(a, e);
			...
			CKEDITOR.plugins.add('magicline', {
					...
					a.on('contentDom', function () {
						...
						k.is(H.$inline) || (l.inInlineMode && !q(k) && k.setStyles({
							...
							if ('wysiwyg' == a.mode && !a.readOnly && !z) {
								...
								z = setTimeout(function () {
									...
									z = l.trigger = null;
									C(l);
									t && !l.hiddenMode && a.focusManager.hasFocus && !l.line.mouseNear() && (l.element = W(l, !0)) && ((l.trigger = u(l) || r(l) || Y(l)) && !w(l, l.trigger.upper || l.trigger.lower) ? l.line.attach().place() : (l.trigger = null, l.line.detach()), t = !1)
								}, 30)
							}
						...
						[
							...
							for (d in {
								...
								br: 1,
								div: 1
							}) for (b = a.document.getElementsByTag(d), e = b.count(); e--;) if ((c = b.getItem(e)).data('cke-magicline-hot')) {
								l.hotNode = c;
			...
			}); var A = CKEDITOR.tools.extend, G = CKEDITOR.dom.element, D = G.createFromHtml, F = CKEDITOR.env, I = CKEDITOR.env.ie && 9 > CKEDITOR.env.version, H = CKEDITOR.dtd, K = {
			}, J = 128, E = 64, R = 32, O = 16, S = 4, L = 2, V = 1, Z = ' ', X = H.$listItem, da = H.$tableContent, P = A({
			}, H.$nonEditable, H.$empty), Q = H.$block, M = 100, U = 'width:0px;height:0px;padding:0px;margin:0px;display:block;z-index:9999;color:#fff;position:absolute;font-size: 0px;line-height:0px;', T = U + 'border-color:transparent;display:block;border-style:solid;', N = '<span>' + Z + '</span>'; K[CKEDITOR.ENTER_BR] = 'br'; K[CKEDITOR.ENTER_P] = 'p'; K[CKEDITOR.ENTER_DIV] = 'div'; d.prototype = {
				set: function (a, b, c) {
				...
				function b(d) {
					...
					for (var f = Number.MAX_VALUE, m, N, p, r; h && !h.equals(g) && (N = g.getNext(d.isRelevant));) m = Math.abs(c(d, g, N) - d.mouse.y),
						m < f && (f = m, p = g, r = N),
						g = N,
						x(d, g);
						...
						a.addCommand('maximize', {
							...
							exec: function () {
								...
								if (this.state == CKEDITOR.TRISTATE_OFF) {
									...
									for (t = a.container; t = t.getParent();) t.setCustomData('maximize_saved_styles', b(t)),
										t.setStyle('z-index', a.config.baseFloatZIndex - 5);
									...
									for (var t = [
										w,
										v
									], r = 0; r < t.length; r++) c(t[r], t[r].getCustomData('maximize_saved_styles')),
										t[r].removeCustomData('maximize_saved_styles');
									for (t = a.container; t = t.getParent();) c(t, t.getCustomData('maximize_saved_styles')),
								...
								this.toggleState();
								if (t = this.uiItems[0]) w = this.state == CKEDITOR.TRISTATE_OFF ? e.maximize.maximize : e.maximize.minimize,
									t = CKEDITOR.document.getById(t._.id),
									t.getChild(1).setHtml(w),
									t.setAttribute('title', w),
									t.setAttribute('href', 'javascript:void("' + w + '");');
			...
			CKEDITOR.plugins.pagebreakCmd = {
				exec: function (d) {
					var b = d.document.createElement('div', {
						attributes: a(d.lang.pagebreak.alt)
					});
					d.insertElement(b)
				},
				context: 'div',
					...
					d.on('paste', function (b) {
						...
						if (h && (c || l) && (!1 !== d.fire('pasteFromWord', f) || c)) {
							...
							var n = a(d, g, function () {
								if (n) d.fire('paste', e);
								else if (!d.config.pasteFromWordPromptCleanup || c || confirm(d.lang.pastefromword.confirmCleanup)) f.dataValue = CKEDITOR.cleanWord(f.dataValue, d),
									d.fire('afterPasteFromWord', f),
									e.dataValue = f.dataValue,
									!0 === d.config.forcePasteAsPlainText ? e.type = 'text' : CKEDITOR.env.ie && 'allow-word' === d.config.forcePasteAsPlainText && (e.type = 'html');
								c = 0
							});
							n && b.cancel()
						}
			...
			var a = {
				...
				exec: function (a, b) {
					var c = a.lang,
						...
						c = l && 'string' === typeof l ? l : c.pastetext.pasteNotification.replace(/%1/, '<kbd aria-label="' + g.aria + '">' + g.display + '</kbd>');
					a.execCommand('paste', {
						type: 'text',
						notification: l ? c : !1
					})
			...
			var a,
				d = {
					...
					exec: function (b) {
						var c,
							d = b.config,
							l = d.baseHref ? '<base href="' + d.baseHref + '"/>' : '';
						if (d.fullPage) c = b.getData().replace(/<head>/, '$&' + l).replace(/[^>]*(?=<\/title>)/, '$& &mdash; ' + b.lang.preview.preview);
						else {
							var d = '<body ',
								k = b.document && b.document.getBody();
							k && (k.getAttribute('id') && (d += 'id="' + k.getAttribute('id') + '" '), k.getAttribute('class') && (d += 'class="' + k.getAttribute('class') + '" '));
							d += '>';
							c = b.config.docType + '<html dir="' + b.config.contentsLangDirection + '"><head>' + l + '<title>' +
								b.lang.preview.preview + '</title>' + CKEDITOR.tools.buildStyleHtml(b.config.contentsCss) + '</head>' + d + b.getData() + '</body></html>'
						}
						l = 640;
						d = 420;
						k = 80;
						try {
							var e = window.screen,
								l = Math.round(0.8 * e.width),
								d = Math.round(0.7 * e.height),
								k = Math.round(0.1 * e.width)
						} catch (h) {
						}
						...
						var e = '',
							m;
						CKEDITOR.env.ie && (window._cke_htmlToLoad = b.dataValue, m = 'javascript:void( (function(){document.open();' + ('(' + CKEDITOR.tools.fixDomain + ')();').replace(/\/\/.*?\n/g, '').replace(/parent\./g, 'window.opener.') + 'document.write( window.opener._cke_htmlToLoad );document.close();window.opener._cke_htmlToLoad = null;})() )', e = '');
						CKEDITOR.env.gecko && (window._cke_htmlToLoad = b.dataValue, e = CKEDITOR.getUrl(a + 'preview.html'));
						e = window.open(e, null, 'toolbar=yes,location=no,status=yes,menubar=yes,scrollbars=yes,resizable=yes,width=' + l + ',height=' + d + ',left=' + k);
						CKEDITOR.env.ie && e && (e.location = m);
						CKEDITOR.env.ie || CKEDITOR.env.gecko || (m = e.document, m.open(), m.write(b.dataValue), m.close());
						return !0
					}
				};
		...
		CKEDITOR.plugins.removeformat = {
			commands: {
				removeformat: {
					exec: function (a) {
						for (var d = a._.removeFormatRegex || (a._.removeFormatRegex = new RegExp('^(?:' + a.config.removeFormatTags.replace(/,/g, '|') + ')$', 'i')), b = a._.removeAttributes || (a._.removeAttributes = a.config.removeFormatAttributes.split(',')), c = CKEDITOR.plugins.removeformat.filter, g = a.getSelection().getRanges(), l = g.createIterator(), k = function (a) {
							return a.type == CKEDITOR.NODE_ELEMENT
						}, e; e = l.getNextRange();) {
							...
							if (f) for (n(f), m = m.getNextSourceNode(!0, CKEDITOR.NODE_ELEMENT); m && !m.equals(f);) if (m.isReadOnly()) {
								...
								m = m.getNext(k)
							} else n = m.getNextSourceNode(!1, CKEDITOR.NODE_ELEMENT),
								'img' == m.getName() && m.data('cke-realelement') || !c(a, m) || (d.test(m.getName()) ? m.remove(1) : (m.removeAttributes(b), a.fire('removeFormatCleanup', m))),
								m = n;
							e.moveToBookmark(h)
						}
						a.forceNextSelectionCheck();
			...
			filter: function (a, d) {
				for (var b = a._.removeFormatFilters || [
				], c = 0; c < b.length; c++) if (!1 === b[c](d)) return !1;
				return !0
			}
		},
		...
		CKEDITOR.plugins.add('resize', {
			init: function (a) {
				...
				function b() {
					...
					a.document && (a.document.removeListener('mousemove', d), a.document.removeListener('mouseup', b))
				}
				var c = a.config,
					g = a.ui.spaceId('resizer'),
					l = a.element ? a.element.getDirection(1) : 'ltr';
				!c.resize_dir && (c.resize_dir = 'vertical');
				void 0 === c.resize_maxWidth && (c.resize_maxWidth = 3000);
				void 0 === c.resize_maxHeight && (c.resize_maxHeight = 3000);
				...
				if (!1 !== c.resize_enabled) {
					...
					a.on('uiSpace', function (b) {
						if ('bottom' == b.data.space) {
							...
							var e = '<span id="' + g + '" class="cke_resizer' + c + ' cke_resizer_' + l + '" title="' + CKEDITOR.tools.htmlEncode(a.lang.common.resize) + '" onmousedown="CKEDITOR.tools.callFunction(' + n + ', event)">' + ('ltr' == l ? '◢' : '◣') + '</span>';
		...
		CKEDITOR.plugins.add('scayt', {
			...
			init: function (a) {
				...
				a.contextMenu && a.addMenuItems && (a.contextMenu.addListener(function (b, c) {
					var e = a.scayt,
						g,
						m;
					e && (m = e.getSelectionNode()) && (g = d.menuGenerator(a, m), e.showBanner('.' + a.contextMenu._.definition.panel.className.split(' ').join(' .')));
					return g
			...
			addMenuItems: function (a) {
				...
				if ((c = [
					...
					'grayt_suggest',
					'grayt_control'
				].concat(c)) && c.length) for (g = 0; g < c.length; g++) a.addMenuGroup(c[g], g - 10);
			...
			bindEvents: function (a) {
					...
					l = function () {
						...
						c.attachListener(c, 'focus', function (c) {
							...
							if ((b || c) && a._.savedSelection) {
								...
								for (var k = 0; k < c.length; k++) d = c[k],
									'string' === typeof d.startContainer.$.nodeValue && (e = d.startContainer.getText().length, (e < d.startOffset || e < d.endOffset) && a.unlockSelection(!1))
							}
						}, this, null, - 10)
					},
					k = function () {
				...
				a.on('readOnly', function (b) {
					var c;
					b && (c = a.scayt, !0 === b.editor.readOnly ? c && c.fire('removeMarkupInDocument', {
					}) : c ? d.reloadMarkup(c) : 'wysiwyg' == b.editor.mode && !0 === d.state.scayt[b.editor.name] && (d.createScayt(a), b.editor.fire('scaytButtonState', CKEDITOR.TRISTATE_ON)))
				});
			...
			parseConfig: function (a) {
				...
				if (a.config.scayt_disableOptionsStorage) {
					var d = CKEDITOR.tools.isArray(a.config.scayt_disableOptionsStorage) ? a.config.scayt_disableOptionsStorage : 'string' === typeof a.config.scayt_disableOptionsStorage ? [
			...
			buildSuggestionMenuItems: function (a, d, b) {
				...
				} else for (b = 0; b < d.length; b++) h = 'grayt_suggest_' + CKEDITOR.plugins.scayt.suggestions[b].replace(' ', '_'),
					a.addCommand(h, this.createCommand(CKEDITOR.plugins.scayt.suggestions[b], l, k)),
					a.addMenuItem(h, {
						label: d[b],
						command: h,
				...
				this.loadScaytLibrary(a, function (a) {
					...
					if (l) {
						...
						k.subscribe('suggestionListSend', function (a) {
							...
							], e = 0; e < a.suggestionList.length; e++) b['word_' + a.suggestionList[e]] || (b['word_' + a.suggestionList[e]] = a.suggestionList[e], c.push(a.suggestionList[e]));
					...
					a.addCommand('selectAll', {
						...
						exec: function (a) {
							...
							if (b.is('textarea')) a = b.$,
								CKEDITOR.env.ie && a.createTextRange ? a.createTextRange().execCommand('SelectAll') : (a.selectionStart = 0, a.selectionEnd = a.value.length),
								a.focus();
							else {
			...
			CKEDITOR.plugins.add('showblocks', {
				onLoad: function () {
					var a = 'p div pre address blockquote h1 h2 h3 h4 h5 h6'.split(' '),
						...
						h = e ? ':not([contenteditable=false]):not(.cke_show_blocks_off)' : '',
						m,
						f;
					for (b = c = g = l = ''; m = a.pop();) f = a.length ? ',' : '',
						b += '.cke_show_blocks ' + m + h + f,
						g += '.cke_show_blocks.cke_contents_ltr ' + m + h + f,
						l += '.cke_show_blocks.cke_contents_rtl ' + m + h + f,
						c += '.cke_show_blocks ' + m + h + '{background-image:url(' + CKEDITOR.getUrl(k + 'images/block_' + m + '.png') + ')}';
					...
					e || CKEDITOR.addCss('.cke_show_blocks [contenteditable=false],.cke_show_blocks .cke_show_blocks_off{border:none;padding-top:0;background-image:none}.cke_show_blocks.cke_contents_rtl [contenteditable=false],.cke_show_blocks.cke_contents_rtl .cke_show_blocks_off{padding-right:0}.cke_show_blocks.cke_contents_ltr [contenteditable=false],.cke_show_blocks.cke_contents_ltr .cke_show_blocks_off{padding-left:0}')
				},
				init: function (d) {
					function b() {
						c.refresh(d)
			...
			CKEDITOR.plugins.add('stylescombo', {
				...
				init: function (a) {
					...
					a.on('stylesSet', function (b) {
						if (b = b.data.styles) {
							for (var e, h, m, f = 0, n = b.length; f < n; f++) (e = b[f], a.blockless && e.element in CKEDITOR.dtd.$block || 'string' == typeof e.type && !CKEDITOR.style.customHandlers[e.type] || (h = e.name, e = new CKEDITOR.style(e), a.filter.customConfig && !a.filter.check(e))) || (e._name = h, e._.enterMode = d.enterMode, e._.type = m = e.assignedTo || e.type, e._.weight = f + 1000 * (m == CKEDITOR.STYLE_OBJECT ? 1 : m == CKEDITOR.STYLE_BLOCK ? 2 : 3), c[h] = e, g.push(e), l.push(e));
							g.sort(function (a, b) {
					...
					a.ui.addRichCombo('Styles', {
						...
						init: function () {
							var a,
								...
								n;
							f = 0;
							for (n = g.length; f < n; f++) a = g[f],
								c = a._name,
								l = a._.type,
								l != d && (this.startGroup(b['panelTitle' + String(l)]), d = l),
		...
		function () {
					...
					exec: function (b) {
						if (b.editable().hasFocus) {
							...
							if (e = (new CKEDITOR.dom.elementPath(c.getCommonAncestor(), c.root)).contains({
								...
								if (d || a) if (d) d = new CKEDITOR.dom.element(d),
									c.moveToElementEditStart(d),
									c.checkStartOfBlock() && c.checkEndOfBlock() || c.selectNodeContents(d);
								else return !0;
								else {
									for (var m = e.getAscendant('table').$, d = e.getParent().$.cells, m = new CKEDITOR.dom.element(m.insertRow(- 1), b.document), f = 0, n = d.length; f < n; f++) m.append((new CKEDITOR.dom.element(d[f], b.document)).clone(!1, !1)).appendBogus();
			...
			CKEDITOR.plugins.add('tab', {
				init: function (g) {
					for (var l = !1 !== g.config.enableTabKeyTools, k = g.config.tabSpaces || 0, e = ''; k--;) e += ' ';
									...
									right: function (a) {
										if (a.styles) {
											...
											if (a.styles.border) b = CKEDITOR.tools.style.parse.border(a.styles.border);
												...
												var d = a.styles;
												d['border-left'] && d['border-left'] === d['border-right'] && d['border-right'] === d['border-top'] && d['border-top'] === d['border-bottom'] && (b = CKEDITOR.tools.style.parse.border(d['border-top']))
											}
					...
					a.addCommand('tableDelete', d({
						exec: function (a) {
							...
							if (b) {
								var d = b.getParent(),
									k = a.editable();
								1 != d.getChildCount() || d.is('td', 'th') || d.equals(k) || (b = d);
								a = a.createRange();
		...
		function () {
			function a(a, b) {
				...
				function e(a) {
					0 < d.length || a.type != CKEDITOR.NODE_ELEMENT || !v.test(a.getName()) || a.getCustomData('selected_cell') || (CKEDITOR.dom.element.setMarker(f, a, 'selected_cell', !0), d.push(a))
				}
				var d = [
				],
					f = {
			...
			function d(b, c) {
				for (var e = w(b) ? b : a(b), d = e[0], f = d.getAscendant('table'), d = d.getDocument(), g = e[0].getParent(), h = g.$.rowIndex, e = e[e.length - 1], k = e.getParent().$.rowIndex + e.$.rowSpan - 1, e = new CKEDITOR.dom.element(f.$.rows[k]), h = c ? h : k, g = c ? g : e, e = CKEDITOR.tools.buildTableMap(f), f = e[h], h = c ? e[h - 1] : e[h + 1], e = e[0].length, d = d.createElement('tr'), k = 0; f[k] && k < e; k++) {
					var l;
					1 < f[k].rowSpan && h && f[k] == h[k] ? (l = f[k], l.rowSpan += 1) : (l = (new CKEDITOR.dom.element(f[k])).clone(), l.removeAttribute('rowSpan'), l.appendBogus(), d.append(l), l = l.$);
					k += l.colSpan - 1
				}
				...
				if (c instanceof CKEDITOR.dom.selection) {
					...
					for (c = h; c <= k; c++) {
						for (var l = g[c], m = new CKEDITOR.dom.element(f.$.rows[c]), n = 0; n < l.length; n++) {
							...
							1 == p.$.rowSpan ? p.remove() : (--p.$.rowSpan, q == c && (q = g[c + 1], q[n - 1] ? p.insertAfter(new CKEDITOR.dom.element(q[n - 1])) : (new CKEDITOR.dom.element(f.$.rows[c + 1])).append(p, 1)));
							n += p.$.colSpan - 1
						}
						d.push(m)
					}
			...
			function l(b, c) {
				for (var e = w(b) ? b : a(b), d = e[0].getAscendant('table'), f = g(e, 1), e = g(e), h = c ? f : e, k = CKEDITOR.tools.buildTableMap(d), d = [
				], f = [
				], e = [
				], l = k.length, m = 0; m < l; m++) d.push(k[m][h]),
					f.push(c ? k[m][h - 1] : k[m][h + 1]);
				for (m = 0; m < l; m++) d[m] && (1 < d[m].colSpan && f[m] == d[m] ? (k = d[m], k.colSpan += 1) : (h = new CKEDITOR.dom.element(d[m]), k = h.clone(), k.removeAttribute('colSpan'), k.appendBogus(), k[c ? 'insertBefore' : 'insertAfter'].call(k, h), e.push(k), k = k.$), m += k.rowSpan - 1);
				return e
			}
			function k(b) {
				function c(a) {
				...
				b.reset();
				var n = 0;
				for (b = h.length; n < b; n++) for (var p = 0, q = h[n].length; p < q; p++) void 0 === k && h[n][p] == f.$ && (k = p),
					h[n][p] == g.$ && (l = p);
				for (n = k; n <= l; n++) for (p = 0; p < h.length; p++) g = h[p],
					f = new CKEDITOR.dom.element(d.$.rows[p]),
					g = new CKEDITOR.dom.element(g[n]),
					g.$ && (1 == g.$.colSpan ? g.remove() : --g.$.colSpan, p += g.$.rowSpan - 1, f.$.cells.length || m.push(f));
			...
			function h(b) {
				if (b instanceof CKEDITOR.dom.selection) {
					...
					a: {
						...
						f = e.length - 1;
						for (var k = {
						}, l, n; l = e[g++];) CKEDITOR.dom.element.setMarker(k, l, 'delete_cell', !0);
			...
			function n(b, c, e) {
				...
				for (c = 0; c < d.length; c++) {
					...
					var R = h.getParent(),
						...
						v = Math.max(v, V - p + S);
					q = Math.max(q, R - n + L);
					e || (S = h, (L = S.getBogus()) && L.remove(), S.trim(), h.getChildren().count() && (R == K || !O || O.isBlockBoundary && O.isBlockBoundary({
						br: 1
				...
				d = e.count();
				for (c = d - 1; 0 <= c; c--) g = e.getItem(c),
					g.$.cells.length || (g.remove(), d++);
				return b
			}
			function p(b, c) {
				var e = a(b);
				...
				if (1 < m) {
					...
					p || g.append(d)
				} else for (m = n = 1, g = d.clone(), g.insertAfter(d), g.append(d = e.clone()), p = f(h, k), l = 0; l < p.length; l++) p[l].rowSpan++;
				d.appendBogus();
				e.$.rowSpan = n;
				d.$.rowSpan = m;
		...
		function () {
			...
			function d(a, b) {
				...
				l > n && (c = l, l = n, n = c);
				for (c = f; c <= g; c++) for (f = l; f <= n; f++) e = new CKEDITOR.dom.element(d[c][f]),
					e.$ && !e.getCustomData('selected_cell') && (h.push(e), CKEDITOR.dom.element.setMarker(k, e, 'selected_cell', !0));
				CKEDITOR.dom.element.clearAllMarkers(k);
				return h
			}
			function b(a) {
				if (a) return a = a.clone(),
			...
			function l(a, b) {
				var c = [
					...
					e,
					d;
				for (d = 0; d < b.length; d++) e = a.createRange(),
			...
			function e(b, c, e) {
				...
				c = c.is('table') ? null : c;
				var h;
				(h = t.active && !t.first) && !(h = c) && (h = b.getSelection().getRanges(), h = 1 < f.length || h[0] && !h[0].collapsed ? !0 : !1);
				if (h) t.first = c || f[0],
					t.dirty = c ? !1 : 1 !== f.length;
				else if (t.active && c && t.first.getAscendant('table').equals(c.getAscendant('table'))) {
					f = d(t.first, c);
					if (!t.dirty && 1 === f.length && !a(e.data.getTarget())) return g(b, 'mouseup' === e.name);
					t.dirty = !0;
					t.last = c;
					l(b, f)
				}
			...
			function f(b) {
				...
				if (b.data.getTarget().getName && ('mouseup' === b.name || !a(b.data.getTarget()))) {
					...
					l(b, n, p, q) && g(m, !0);
					!t.active && 'mousedown' ===
						b.name && CKEDITOR.tools.getMouseButton(b) === CKEDITOR.MOUSE_BUTTON_LEFT && q && (t = {
							active: !0
						}, CKEDITOR.document.on('mouseup', f, null, {
							editor: m
						}));
					(v || q) && e(m, v || q, b);
					'mouseup' === b.name && (CKEDITOR.tools.getMouseButton(b) === CKEDITOR.MOUSE_BUTTON_LEFT && (h(b.data.getTarget()) || d(p, q)) && k(m), t = {
						active: !1
			...
			function w(a, b, c) {
				a.on('beforeCommandExec', function (c) {
					- 1 !== CKEDITOR.tools.array.indexOf(b, c.data.name) && (c.data.selectedCells = r(a.getSelection()))
				});
			...
			v.prototype.insertRow = function (a, b, c) {
				...
				for (var e = this.cells.first.$.cellIndex, d = this.cells.last.$.cellIndex, f = c ? [
				] : this.cells.all, g, h = 0; h < a; h++) g = x(c ? this.cells.all : f, b),
					g = CKEDITOR.tools.array.filter(g.find('td, th').toArray(), function (a) {
			...
			var C = {
				onPaste: function (a) {
					...
					var f = a.editor,
						...
						n,
						p;
					!h.length || 1 === h.length && !b(g.getRanges()[0]) && !m || m && !k || (h = h[0].getAscendant('table'), n = new v(r(g, h)), f.once('afterPaste', function () {
						var a;
						if (p) {
					...
					c.attachListener(c, 'keydown', function (a) {
						function c(b, e) {
							...
							if (k && k.is && k.is(d)) {
								...
								else f.setStartBefore(k.getAscendant('table', !0)),
									f.collapse(!0);
								g[0].deleteContents();
								return [f]
							}
							if (k) return f.moveToElementEditablePosition(k),
								[
									f
								]
						}
						...
						return function (d) {
							var f = d.data.getKey(),
								...
								l,
								m;
							if (e[f] && (g = a.getSelection()) && g.isInTable() && g.isFake) if (k = g.getRanges(), l = k[0]._getTableElement(), m = k[k.length -
								1]._getTableElement(), d.data.preventDefault(), d.cancel(), 8 < f && 46 > f) k[0].moveToElementEditablePosition(h ? l : m, !h),
									g.selectRanges([k[0]]);
							else {
		...
		function () {
			...
			CKEDITOR.loadTemplates = function (a, c) {
				for (var g = [
				], l = 0, k = a.length; l < k; l++) d[a[l]] || (g.push(a[l]), d[a[l]] = 1);
		...
		function () {
			...
			l.prototype = {
				onKeydown: function (d) {
					var g = d.data.getKey();
					if (229 !== g) if (- 1 < CKEDITOR.tools.indexOf(a, d.data.getKeystroke())) d.data.preventDefault();
					else if (this.keyEventsStack.cleanUp(d), d = this.undoManager, this.keyEventsStack.getLast(g) || this.keyEventsStack.push(g), this.lastKeydownImage = new c(d.editor), b.isNavigationKey(g) || this.undoManager.keyGroupChanged(g)) if (d.strokesRecorded[0] || d.strokesRecorded[1]) d.save(!1, this.lastKeydownImage, !1),
						d.resetType()
				},
				onInput: function () {
				...
				onKeyup: function (a) {
					...
					if (!(b.ieFunctionalKeysBug(a) && this.lastKeydownImage && this.lastKeydownImage.equalsContent(new c(d.editor, !0)))) if (0 < g) d.type(a);
		...
		function () {
			...
			function b(a, b) {
				...
				a.on('destroy', k);
				this.lineTpl = (new CKEDITOR.template('<div data-cke-lineutils-line="1" class="cke_reset_all" style="{lineStyle}"><span style="{tipLeftStyle}">&nbsp;</span><span style="{tipRightStyle}">&nbsp;</span></div>')).output({
					...
					tipRightStyle: CKEDITOR.tools.writeCssText(CKEDITOR.tools.extend({
					}, g, {
							right: '0px',
			...
			a.prototype = {
				start: function (a) {
					var b = this,
						...
						k,
						l,
						t = CKEDITOR.tools.eventsBuffer(50, function () {
							c.readOnly || 'wysiwyg' != c.mode || (b.relations = {
							}, (g = d.$.elementFromPoint(k, l)) && g.nodeType && (e = new CKEDITOR.dom.element(g), b.traverseSearch(e), isNaN(k + l) || b.pixelSearch(e, k, l), a && a(b.relations, k, l)))
						});
				...
				pixelSearch: function () {
					function a(d, e, g, h, k) {
						for (var l = 0, t; k(g);) {
							g += h;
							if (25 == ++l) break;
							if (t = this.doc.$.elementFromPoint(e, g)) if (t == d) l = 0;
				...
				locate: function () {
					...
					return function (b) {
						...
						for (var d in b) c = b[d],
							...
							c.type & CKEDITOR.LINEUTILS_BEFORE && this.store(d, CKEDITOR.LINEUTILS_BEFORE, a(c, CKEDITOR.LINEUTILS_BEFORE)),
							c.type & CKEDITOR.LINEUTILS_AFTER && this.store(d, CKEDITOR.LINEUTILS_AFTER, a(c, CKEDITOR.LINEUTILS_AFTER)),
							c.type & CKEDITOR.LINEUTILS_INSIDE && this.store(d, CKEDITOR.LINEUTILS_INSIDE, (c.elementRect.top + c.elementRect.bottom) / 2);
			...
			b.prototype = {
				...
				getStyle: function (a, b) {
					...
					this.inline ? e.left = c.elementRect.left - this.rect.relativeX : (0 < c.elementRect.left ? e.left = this.rect.left + c.elementRect.left : (e.width += c.elementRect.left, e.left = this.rect.left), 0 < (c = e.left + e.width - (this.rect.left + this.winPane.width)) && (e.width -= c));
			...
			CKEDITOR.plugins.widgetselection = {
				...
				addFillers: function (d) {
					...
					if (!this.isWholeContentSelected(d) && 0 < d.getChildCount()) {
						...
						if (this.hasFiller(d)) return b = b.createRange(),
				...
				removeFillers: function (a) {
					if (this.hasFiller(a) && !this.isWholeContentSelected(a)) {
						var b = a.findOne(this.fillerTagName + '[' + this.fillerAttribute + '=start]'),
							c = a.findOne(this.fillerTagName + '[' + this.fillerAttribute + '=end]');
				...
				removeFiller: function (a, b) {
					if (a) {
						...
						a.remove();
						k && 0 < k.length && k != this.fillerContent ? (b.insertHtmlIntoRange(k, c.getSelection().getRanges()[0]), l.setStartAt(b.getChild(b.getChildCount() - 1), CKEDITOR.POSITION_BEFORE_END), c.getSelection().selectRanges([l])) : e && (g ? l.setStartAt(b.getFirst().getNext(), CKEDITOR.POSITION_AFTER_START) : l.setEndAt(b.getLast().getPrevious(), CKEDITOR.POSITION_BEFORE_END), b.editor.getSelection().selectRanges([l]))
					}
				},
		...
		function () {
			...
			function d(a, b, c, e, g) {
				...
				CKEDITOR.tools.extend(this, e, {
					...
					_: {
						downcastFn: e.downcast && 'string' == typeof e.downcast ? e.downcasts[e.downcast] : e.downcast
					}
				}, !0);
					...
					exec: function (a, c) {
						...
						if (e && e.name == b.name) e.edit();
							...
							var e = 'function' == typeof b.defaults ? b.defaults() : b.defaults,
								...
								g,
								f = a.widgets.wrapElement(e, b.name),
								k = new CKEDITOR.dom.documentFragment(f.getDocument());
							k.append(f);
							(g = a.widgets.initOn(e, b, c && c.startupData)) ? (e = g.once('edit', function (b) {
							...
							}, null, null, 999), g.edit(), e.removeListener()) : d()
						}
					},
			...
			function g(a, b) {
				function c(a, d) {
					var e = b.upcast.split(','),
						g,
						f;
					for (f = 0; f < e.length; f++) if (g = e[f], g === a.name) return b.upcasts[g].call(this, a, d);
					return !1
				}
				function d(b, c, e) {
					var g = CKEDITOR.tools.getIndex(a._.upcasts, function (a) {
						return a[2] > e
					});
					0 > g && (g = a._.upcasts.length);
					a._.upcasts.splice(g, 0, [
						CKEDITOR.tools.bind(b, c),
						c.name,
						e
					])
				}
						...
						if (a && a.initOnlyNew) c = this.initOnAll();
							...
							for (g = h.count(); e < g; e++) {
								...
								if (k = !this.getByElement(f, !0)) {
									a: {
										k = w;
										for (var l = f; l = l.getParent();) if (k(l)) {
											k = !0;
											break a
										}
										k = !1
			...
			function e(a) {
				if ('undefined' != typeof a.attributes && a.attributes['data-widget']) {
					var b = h(a),
						c = m(a),
						d = !1;
					b && b.value && b.value.match(/^\s/g) && (b.parent.attributes['data-cke-white-space-first'] = 1, b.value = b.value.replace(/^\s/g, '&nbsp;'), d = !0);
					c && c.value && c.value.match(/\s$/g) && (c.parent.attributes['data-cke-white-space-last'] = 1, c.value = c.value.replace(/\s$/g, '&nbsp;'), d = !0);
				...
				return {
					...
					iterator: function (a) {
						var g,
							...
							h,
							l;
						if ('data-cke-widget-wrapper' in a.attributes) return (a = a.getFirst(d.isParserWidgetElement)) && b.push([a]),
							!1;
						if ('data-widget' in a.attributes) return b.push([a]),
							!1;
						if (l = c.length) {
							if (a.attributes['data-cke-widget-upcasted']) return !1;
							h = 0;
							for (g = e.length; h < g; ++h) if (!1 === e[h](a)) return;
							for (h = 0; h < l; ++h) if (g = c[h], k = {
							}, f = g[0](a, k)) return f instanceof CKEDITOR.htmlParser.element && (a = f),
								a.attributes['data-cke-widget-data'] = encodeURIComponent(JSON.stringify(k)),
			...
			function B(a) {
				...
				b.on('dragstart', function (c) {
					var e = c.data.target;
					d.isDomDragHandler(e) && (e = a.getByElement(e), c.data.dataTransfer.setData('cke/widget-id', e.id), b.focus(), e.focus())
				});
			...
			function x(a) {
				...
				b.on('contentDom', function () {
					...
					c.attachListener(e, 'mousedown', function (c) {
						...
						f = 0;
						g && (g.inline && e.type == CKEDITOR.NODE_ELEMENT && e.hasAttribute('data-cke-widget-drag-handler') ? (f = 1, a.focused != g && b.getSelection().removeAllRanges()) : d.getNestedEditable(g.wrapper, e) ? g = null : (c.data.preventDefault(), CKEDITOR.env.ie || g.focus()))
					});
					c.attachListener(e, 'mouseup', function () {
						f && g && g.wrapper && (f = 0, g.focus())
					});
					CKEDITOR.env.ie && c.attachListener(e, 'mouseup', function () {
						setTimeout(function () {
							g && g.wrapper && c.contains(g.wrapper) && (g.focus(), g = null)
						})
					})
				});
				b.on('doubleclick', function (b) {
					var c = a.getByElement(b.data.element);
					if (c && !d.getNestedEditable(c.wrapper, b.data.element)) return c.fire('doubleclick', {
			...
			function z(a) {
				...
				b.on('selectionChange', function (c) {
					var e = (c = d.getNestedEditable(b.editable(), c.data.selection.getStartElement())) && a.getByElement(c),
				...
				b.on('toDataFormat', function (b) {
					...
					b.data.dataValue.forEach(function (b) {
						...
						if ('data-cke-widget-id' in c) {
							if (c = a.instances[c['data-cke-widget-id']]) e = b.getFirst(d.isParserWidgetElement),
								g.push({
									wrapper: b,
									element: e,
									widget: c,
							...
							1].editables[c['data-cke-widget-editable']] = b,
							!1
					}, CKEDITOR.NODE_ELEMENT, !0)
				}, null, null, 8);
				b.on('toDataFormat', function (a) {
					if (a.data.downcastingSessionId) {
						...
						for (var b, d, e, g, f, k; b = a.shift();) {
							d = b.widget;
							e = b.element;
							g = d._.downcastFn && d._.downcastFn.call(d, e);
							for (k in b.editables) f = b.editables[k],
								delete f.attributes.contenteditable,
								f.setHtml(d.editables[k].getData());
							g || (g = e);
							b.wrapper.replaceWith(g)
						}
					}
				}, null, null, 13);
			...
			function G(a) {
				...
				b.on('toHtml', function (b) {
					var e = n(a),
						g;
					for (b.data.dataValue.forEach(e.iterator, CKEDITOR.NODE_ELEMENT, !0); g = e.toBeWrapped.pop();) {
						var f = g[0],
							k = f.parent;
				...
				b.on('dataReady', function () {
					if (e) for (var c = b.editable().find('.cke_widget_wrapper'), g, f, k = 0, h = c.count(); k < h; ++k) g = c.getItem(k),
						f = g.getFirst(d.isDomWidgetElement),
						f.type == CKEDITOR.NODE_ELEMENT && f.data('widget') ? (f.replace(g), a.wrapElement(f)) : g.remove();
					e = 0;
					a.destroyAll(!0);
					a.initOnAll()
				});
				...
				b.on('paste', function (a) {
					...
					a.range && (a = d.getNestedEditable(b.editable(), a.range.startContainer)) && (a = CKEDITOR.filter.instances[a.data('cke-filter')]) && b.setActiveFilter(a)
				});
			...
			function D(a) {
				...
				return {
					...
					commit: function () {
						...
						for (g && (f = a.focused) && l(a, f); f = d.pop();) b.splice(CKEDITOR.tools.indexOf(b, f), 1),
							f.isInited() && (k = f.editor.checkDirty(), f.setSelected(!1), !k && f.editor.resetDirty());
						g && e && (k = a.editor.checkDirty(), a.focused = e, a.fire('widgetFocused', {
							widget: e
						}), e.setFocused(!0), !k && a.editor.resetDirty());
						for (; f = c.pop();) b.push(f),
							f.setSelected(!0);
			...
			function F(a, b, c) {
				...
				var e = a.data.classes || {
					...
					g;
				if (b) {
					for (e = CKEDITOR.tools.clone(e); g = b.pop();) c ? e[g] || (d = e[g] = 1) : e[g] && (delete e[g], d = 1);
			...
			function H(a, b) {
				...
				if (!d.getById('cke_copybin')) {
					...
					e.setHtml('<span data-cke-copybin-start="1">​</span>' + c.editable().getHtmlFromRange(k).getHtml() + '<span data-cke-copybin-end="1">​</span>');
					c.fire('saveSnapshot');
					c.fire('lockSnapshot');
					f.append(e);
					c.editable().append(f);
			...
			function S(a) {
				if (a.draggable) {
					...
					if (!a.inline && (e.on('mousedown', L, a), CKEDITOR.env.ie && 9 > CKEDITOR.env.version)) e.on('dragstart', function (a) {
						a.data.preventDefault(!0)
					});
			...
			function L(a) {
				function b() {
					...
					CKEDITOR.tools.isEmpty(g.visible) || (d = e.getRange(d[0]), this.focus(), f.fire('drop', {
						dropRange: d,
				...
				if (CKEDITOR.tools.getMouseButton(a) === CKEDITOR.MOUSE_BUTTON_LEFT) {
					...
					k.push(f.on('mousemove', function (a) {
						m = a.data.$.clientY;
						p.input()
					}));
					g.fire('dragstart', {
					...
					f.isInline() || k.push(CKEDITOR.document.once('mouseup', b, this))
				}
			}
			function V(a) {
				var b,
				...
				if (a.editables) for (b in d) c = d[b],
					a.initEditable(b, 'string' == typeof c ? {
						selector: c
					}
						: c)
			}
			...
			function M() {
				...
				CKEDITOR.style.addCustomHandler({
					...
					setup: function (a) {
						...
						if (this.group = 'string' == typeof a.group ? [
							a.group
						] : a.group) {
							a = this.widget;
							var b;
							...
							for (var d = 0, e = this.group.length; d < e; d++) b = this.group[d],
								c[a][b] || (c[a][b] = [
								]),
								c[a][b].push(this)
						}
					...
					removeStylesFromSameGroup: function (a) {
						...
						d = a.elementPath();
						if (this.checkApplicable(d, a)) for (var g = 0, f = this.group.length; g < f; g++) {
				...
				afterInit: function (a) {
					if (void 0 !== CKEDITOR.document.$.querySelectorAll) {
						var b = a.widgets.registered,
							...
							e;
						for (d in b) c = b[d],
							(e = c.button) && a.ui.addButton && a.ui.addButton(CKEDITOR.tools.capitalize(c.name, !0), {
								label: e,
								command: c.name,
								toolbar: 'insert,10'
							});
			...
			a.prototype = {
				...
				destroyAll: function (a, b) {
					...
					if (b && !a) {
						...
						for (var e = d.count(), g = 0; g < e; ++g) (c = this.getByElement(d.getItem(g), !0)) && this.destroy(c)
				...
				initOnAll: function (a) {
					...
					for (var b = [
					], c, e = a.count(); e--;) (c = this.initOn(a.getItem(e).getFirst(d.isDomWidgetElement))) && b.push(c);
					return b
				},
				onWidget: function (a) {
				...
				parseElementClasses: function (a) {
					...
					for (var b, c = {
					}, d = 0; b = a.pop();) - 1 == b.indexOf('cke_') && (c[b] = d = 1);
					return d ?
						c : null
				},
			...
			d.prototype = {
				...
				destroyEditable: function (a, b) {
					...
					b || (this.repository.destroyAll(!1, c), c.removeClass('cke_widget_editable'), c.removeClass('cke_widget_editable_focused'), c.removeAttributes(['contenteditable',
				...
				initEditable: function (a, c) {
					...
					}), d.filter && d.data('cke-filter', d.filter.id), d.addClass('cke_widget_editable'), d.removeClass('cke_widget_editable_focused'), c.pathName && d.data('cke-display-name', c.pathName), this.editor.focusManager.add(d), d.on('focus', E, this), CKEDITOR.env.ie && d.on('blur', J, this), d._.initialSetData = !0, d.setData(d.getHtml()), !0) : !1
				},
				_findOneNotNested: function (a) {
					a = this.wrapper.find(a);
					for (var b, c, e = 0; e < a.count(); e++) if (b = a.getItem(e), c = b.getAscendant(d.isDomWidgetWrapper), this.wrapper.equals(c)) return b;
					return null
				},
				isInited: function () {
			...
			CKEDITOR.tools.extend(CKEDITOR.fileTools, {
						...
						init: function () {
							...
							d.on('update', function (g) {
								if (b.wrapper && b.wrapper.getParent()) {
									a.fire('lockSnapshot');
									g = 'on' + l(d.status);
									if ('function' !== typeof b[g] || !1 !== b[g](d)) n = 'cke_upload_' + d.status,
										b.wrapper && n != f && (f && b.wrapper.removeClass(f), b.wrapper.addClass(n), f = n),
										'error' != d.status && 'abort' != d.status || a.widgets.del(b);
									a.fire('unlockSnapshot')
								} else a.editable().find('[data-cke-upload-id="' +
									c + '"]').count() || d.abort(),
									g.removeListener()
							});
							d.update()
				...
				bindNotifications: function (a, d) {
					function b() {
						...
						if (!c || c.isFinished()) c = a._.uploadWidgetNotificaionAggregator = new CKEDITOR.plugins.notificationAggregator(a, a.lang.uploadwidget.uploadMany, a.lang.uploadwidget.uploadOne),
					...
					d.on('update', function () {
						!g && d.uploadTotal && (b(), g = c.createTask({
							weight: d.uploadTotal
						}));
					...
					if (CKEDITOR.plugins.clipboard.isFileApiSupported) {
						...
						l && (b.addUploadWidget(a, 'uploadimage', {
							...
							fileToElement: function () {
								...
								a.setAttribute('src', 'data:image/gif;base64,R0lGODlhDgAOAIAAAAAAAP///yH5BAAAAAAALAAAAAAOAA4AAAIMhI+py+0Po5y02qsKADs=');
								return a
							},
							parts: {
								img: 'img'
							...
							onUploaded: function (a) {
								...
								this.replaceWith('<img src="' +
									a.url + '" width="' + (a.responseData.width || b.naturalWidth) + '" height="' + (a.responseData.height || b.naturalHeight) + '">')
							}
		...
		function () {
			function a(a) {
				...
				this.fixInitialSelection();
				var f = this;
				CKEDITOR.env.ie && !CKEDITOR.env.edge && e.getDocumentElement().addClass(e.$.compatMode);
				...
				CKEDITOR.env.iOS && this.attachListener(e, 'touchend', function () {
					a.focus()
				});
				...
				init: function (a) {
					...
					a.addMode('wysiwyg', function (b) {
						...
						var e = 'document.open();' + (CKEDITOR.env.ie ? '(' + CKEDITOR.tools.fixDomain + ')();' : '') + 'document.close();',
							...
							h = CKEDITOR.dom.element.createFromHtml('<iframe src="' + e + '" frameBorder="0"></iframe>');
						h.setStyles({
							width: '100%',
							height: '100%'
						});
						...
						if (m) h.on('load', d);
						var f = a.title,
							n = a.fire('ariaEditorHelpLabel', {
							}).label;
						f && (CKEDITOR.env.ie && n && (f += ', ' + n), h.setAttribute('title', f));
						if (n) {
							var f = CKEDITOR.tools.getNextId(),
								p = CKEDITOR.dom.element.createFromHtml('<span id="' + f + '" class="cke_voice_label">' + n + '</span>');
							e.append(p, 1);
							h.setAttribute('aria-describedby', f)
						}
			...
			CKEDITOR.editor.prototype.addContentsCss = function (a) {
				var b = this.config,
					c = b.contentsCss;
				CKEDITOR.tools.isArray(c) || (b.contentsCss = c ? [
					c
				] : [
					]);
					...
					setData: function (a, c) {
						...
						if (c) this.setHtml(a),
							...
							var e = d.config,
								h = e.fullPage,
								m = e.docType,
								f = CKEDITOR.tools.buildStyleHtml(b()).replace(/<style>/, '<style data-cke-temp="1">');
							h || (f += CKEDITOR.tools.buildStyleHtml(d.config.contentsCss));
							var n = e.baseHref ? '<base href="' +
								e.baseHref + '" data-cke-temp="1" />' : '';
							...
							h ? (/<body[\s|>]/.test(a) || (a = '<body>' + a), /<html[\s|>]/.test(a) || (a = '<html>' + a + '</html>'), /<head[\s|>]/.test(a) ? /<title[\s|>]/.test(a) || (a = a.replace(/<head[^>]*>/, '$&<title></title>')) : a = a.replace(/<html[^>]*>/, '$&<head><title></title></head>'), n && (a = a.replace(/<head[^>]*?>/, '$&' + n)), a = a.replace(/<\/head\s*>/, f + '$&'), a = m + a) : a = e.docType + '<html dir="' + e.contentsLangDirection + '" lang="' + (e.contentsLanguage || d.langCode) + '"><head><title>' + this._.docTitle + '</title>' + n + f + '</head><body' + (e.bodyId ? ' id="' + e.bodyId + '"' : '') + (e.bodyClass ? ' class="' + e.bodyClass + '"' : '') + '>' + a + '</body></html>';
							...
							n && CKEDITOR.env.ie && 10 > CKEDITOR.env.version && (e += '<script id="cke_basetagscrpt">var baseTag = document.querySelector( "base" );baseTag.href = baseTag.href;</script>');
